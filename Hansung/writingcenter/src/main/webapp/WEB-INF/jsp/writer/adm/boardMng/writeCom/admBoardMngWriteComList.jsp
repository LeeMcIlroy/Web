...
function fn_modify(brdSeq){
	$("#brdSeq").val(brdSeq);
	$("#frm").attr("method","post").attr("action","<c:url value='/xabdmxgr/boardMng/writeCom/admBoardMngWriteComModifyView.do'/>").submit();
}

/* 글쓰기 조회*/
function fn_select(brdSeq){
	$("#brdSeq").val(brdSeq);
	$("#frm").attr("method","post").attr("action","<c:url value='/xabdmxgr/boardMng/writeCom/admBoardMngWriteComView.do'/>").submit();
}
...
function fn_delete(){
	...
	if($("input:checkbox[name='listChkBox']").is(":checked")==true){
		if(window.confirm("게시글을 삭제하시겠습니까?")==true){
			...
			$("#brdSeq").val(delSeq);
			$("#frm").attr("method","post").attr("action","<c:url value='/xabdmxgr/boardMng/writeCom/admBoardMngWriteComDelete.do'/>").submit();
		}else{
			return false;
		}
		...
		<div class="content" id="content">
			...
			<div class="title_area">
				<jsp:include page="/WEB-INF/jsp/writer/adm/inc/incPageTitle.jsp">
		            ...
		            <jsp:param name="dept2" value="대회공지사항"/>
            	</jsp:include>
			</div>
			<div class="cont_box">
			<!-- content -->
				<table class="list_tbl_03 mt30" summary="대회공지사항">
					<caption>대회공지사항</caption>
					<colgroup>
						<col style="width:10%" />
						<col style="width:10%" />
						<col style="width:30%" />
					...
					<tbody>
						<c:forEach var="notice" items="${resultList_notice }">
							<tr>
								...
								<td>
									<c:out value="${notice.topicYn eq 'Y'?'주제글 ':'공지'}"/>
								</td>
								<td class="ta_left">
								...
								<td><c:out value="${notice.useYn }"/></td>
								<td><c:out value="${notice.regDate }"/></td>
								<td><c:out value="${notice.brdHitcount }"/></td>
								<td><c:out value="${notice.brdType eq 'WRITE'?'글쓰기':'프레젠테이션' }"/></td>
							</tr>
						</c:forEach>
						<c:forEach var="list" items="${resultList }" varStatus="status">
							<tr>
								...
								<td><c:out value="${list.brdType eq 'WRITE'?'글쓰기':'프레젠테이션' }"/></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
