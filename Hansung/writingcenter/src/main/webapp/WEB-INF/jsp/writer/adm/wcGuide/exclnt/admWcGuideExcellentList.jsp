...
function fn_list(pageNo){
	...
	$("#frm").attr("method", "post").attr("action", "<c:url value='/xabdmxgr/wcGuide/exclnt/admWcGuideExclntList.do'/>").submit();
}

/* 글쓰기 화면 이동 */
function fn_modify(brdSeq){
	$("#brdSeq").val(brdSeq);
	$("#frm").attr("method","post").attr("action","<c:url value='/xabdmxgr/wcGuide/exclnt/admWcGuideExclntModifyView.do'/>").submit();
}

/* 글쓰기 조회*/
function fn_select(brdSeq){
	$("#brdSeq").val(brdSeq);
	$("#frm").attr("method","post").attr("action","<c:url value='/xabdmxgr/wcGuide/exclnt/admWcGuideExclntView.do'/>").submit();
}
...
function fn_delete(){
	...
	if($("input:checkbox[name='listChkBox']").is(":checked")==true){
		if(window.confirm("게시글을 삭제하시겠습니까?")==true){
			...
			$("#brdSeq").val(delSeq);
			$("#frm").attr("method","post").attr("action","<c:url value='/xabdmxgr/wcGuide/exclnt/admWcGuideExclntDelete.do'/>").submit();
		}else{
			return false;
		}
...
<input type="hidden" id="searchType" name="searchType" value="EXCLNT" />
...
<div class="wrap">
	...
	<div class="container">
		...
		<div class="content" id="content">
			...
			<div class="cont_box">
			<!-- content -->
				<table class="list_tbl_03 mt30" summary="우수과제 목록">
