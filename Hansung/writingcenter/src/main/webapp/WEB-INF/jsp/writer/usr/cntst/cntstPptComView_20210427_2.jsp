...
<script>
	...
	// 파일 다운로드
	function fn_download(type, upSeq, cnt){
		var topicYn = $("#topicYn").val();
		if(fileCnt < cnt && topicYn == 'Y'){
			alert('파일을 순서대로 다운로드해 주세요.');
			return;
		}else{
			fileCnt++;
...
<input type="hidden" id="topicYn" value="${boardVO.topicYn }"/>
<div class="wrap">
	<!-- 스킵 네비게이션 -->
		...
		<div class="content" id="content">
			...
			<div class="cont_box">
				<dl class="view_dl mb20">
					...
					<c:if test="${!empty upfileList }">
					<dd class="font12" style="line-height:20px;">첨부파일 : 
						<c:forEach var="list" items="${upfileList }" varStatus="cnt">
							<br/><a href="#" class="txt_udl" onclick="fn_download('CNTST', <c:out value='${list.upSeq }'/>, <c:out value='${cnt.index }'/>); return false;"><c:out value="${list.upOriginFileName }"/></a>
						</c:forEach>
					</dd>
					</c:if>
				</dl>
