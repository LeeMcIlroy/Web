...
function fn_list(pageNo){
	...
	$("#frm").attr("method", "post").attr("action", "<c:url value='/xabdmxgr/boardMng/writeCom/admBoardMngPresentationList.do'/>").submit();
}

/* 글쓰기 화면 이동 */
function fn_modify(brdSeq){
	$("#brdSeq").val(brdSeq);
	$("#frm").attr("method","post").attr("action","<c:url value='/xabdmxgr/boardMng/writeCom/admBoardMngPresentationModifyView.do'/>").submit();
}

/* 글쓰기 조회*/
function fn_select(brdSeq){
	$("#brdSeq").val(brdSeq);
	$("#frm").attr("method","post").attr("action","<c:url value='/xabdmxgr/boardMng/writeCom/admBoardMngPresentationView.do'/>").submit();
}
...
function fn_delete(){
	...
	if($("input:checkbox[name='listChkBox']").is(":checked")==true){
		if(window.confirm("게시글을 삭제하시겠습니까?")==true){
			...
			$("#brdSeq").val(delSeq);
			$("#frm").attr("method","post").attr("action","<c:url value='/xabdmxgr/boardMng/writeCom/admBoardMngPresentationDelete.do'/>").submit();
		}else{
			return false;
		}
...
<input type="hidden" id="searchType" name="searchType" value="PRESENTATION">
		...
		<div class="content" id="content">
			...
			<div class="title_area">
				<jsp:include page="/WEB-INF/jsp/writer/adm/inc/incPageTitle.jsp">
		            ...
		            <jsp:param name="dept2" value="제13회 한성인 프레젠테이션대회"/>
            	</jsp:include>
			</div>
			<div class="cont_box">
			<!-- content -->
				<table class="list_tbl_03 mt30" summary="제15회 한성인 프레젠테이션대회 목록">
