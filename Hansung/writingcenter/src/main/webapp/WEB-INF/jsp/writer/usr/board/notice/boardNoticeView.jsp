...
<script>
	//목록
	function fn_goList(){
		$("#frm").attr("method","post").attr("action","<c:out value='${pageContext.request.contextPath }/usr/board/notice/boardNoticeList.do'/>").submit();
	}
				...
				<dl class="view_dl mb20">
					...
					<dd class="font12">첨부파일 : 
						<c:forEach var="list" items="${upfileList }" varStatus="cnt">
							<a href="#" onclick="fn_download('NOTI', <c:out value='${list.upSeq }'/>); return false;"><c:out value="${list.upOriginFileName }"/></a><c:if test="${cnt.last == false}">,&nbsp;</c:if>
						</c:forEach>
					</dd>
					</c:if>
				</dl>
