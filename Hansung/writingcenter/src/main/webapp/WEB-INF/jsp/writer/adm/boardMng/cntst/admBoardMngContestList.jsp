...
function fn_list(pageNo){
	...
	$("#frm").attr("method", "post").attr("action", "<c:url value='/xabdmxgr/boardMng/cntst/admBoardMngCntstList.do'/>").submit();
}

/* 글쓰기 화면 이동 */
function fn_modify(brdSeq){
	$("#brdSeq").val(brdSeq);
	$("#frm").attr("method","post").attr("action","<c:url value='/xabdmxgr/boardMng/cntst/admBoardMngCntstModifyView.do'/>").submit();
}

/* 글쓰기 조회*/
function fn_select(brdSeq){
	$("#brdSeq").val(brdSeq);
	$("#frm").attr("method","post").attr("action","<c:url value='/xabdmxgr/boardMng/cntst/admBoardMngCntstView.do'/>").submit();
}
...
function fn_delete(){
	...
	if($("input:checkbox[name='listChkBox']").is(":checked")==true){
		if(window.confirm("게시글을 삭제하시겠습니까?")==true){
			...
			$("#brdSeq").val(delSeq);
			$("#frm").attr("method","post").attr("action","<c:url value='/xabdmxgr/boardMng/cntst/admBoardMngCntstDelete.do'/>").submit();
		}else{
			return false;
		}
...
<input type="hidden" id="searchType" name="searchType" value="CNTST">
		...
		<div class="content" id="content">
			...
			<div class="title_area">
				<jsp:include page="/WEB-INF/jsp/writer/adm/inc/incPageTitle.jsp">
		            ...
		            <jsp:param name="dept2" value="대회자료실"/>
            	</jsp:include>
			</div>
			<div class="cont_box">
			<!-- content -->
