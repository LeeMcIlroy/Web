...
<script>
	function fn_select(brdSeq){
		$("#brdSeq").val(brdSeq);
		$("#frm").attr("method","post").attr("action","<c:out value='${pageContext.request.contextPath }/usr/cntst/cntstWriteComView.do'/>").submit();
	}
	
	function fn_list(pageNo){
		$("#pageIndex").val(pageNo);
		$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/usr/cntst/cntstWriteComList.do'/>").submit();
	}
</script>
<body>
	...
	<div class="container">
		...
		<div class="content" id="content">
			...
			<div class="cont_box">
				...
				<table class="list_type01" summary="제15회 한성인 글쓰기대회 목록">
					...
					<tbody>
						<c:forEach var="notice" items="${resultList_notice }">
							<tr>
								<td class="<c:out value="${notice.topicYn eq 'Y'?'topic_tit':'' }"/>">
									<c:out value="${notice.topicYn eq 'Y'?'주제글':'공지' }"/>
								</td>
								<td class="ta_left">
									<a href="#" onclick="fn_select(<c:out value='${notice.brdSeq }'/>); return false;" class="<c:out value="${notice.topicYn eq 'Y'?'topic_tit':'' }"/>">
