...
function fn_modify(brdSeq){
	$("#brdSeq").val(brdSeq);
	$("#frm").attr("method","post").attr("action","<c:url value='/xabdmxgr/wcGuide/writInfo/admWcGuideWritInfoModifyView.do'/>").submit();
}

/* 글쓰기 조회*/
function fn_select(brdSeq){
	$("#brdSeq").val(brdSeq);
	$("#frm").attr("method","post").attr("action","<c:url value='/xabdmxgr/wcGuide/writInfo/admWcGuideWritInfoView.do'/>").submit();
}
...
function fn_delete(){
	...
	if($("input:checkbox[name='listChkBox']").is(":checked")==true){
		if(window.confirm("게시글을 삭제하시겠습니까?")==true){
			...
			$("#brdSeq").val(delSeq);
			$("#frm").attr("method","post").attr("action","<c:url value='/xabdmxgr/wcGuide/writInfo/admWcGuideWritInfoDelete.do'/>").submit();
		}else{
			return false;
		}
		...
		<div class="content" id="content">
			...
			<div class="title_area">
				<jsp:include page="/WEB-INF/jsp/writer/adm/inc/incPageTitle.jsp">
		            ...
		            <jsp:param name="dept2" value="글쓰기정보"/>
            	</jsp:include>
			</div>
			<div class="cont_box">
			<!-- content -->
				<table class="list_tbl_03 mt30" summary="글쓰기정보">
