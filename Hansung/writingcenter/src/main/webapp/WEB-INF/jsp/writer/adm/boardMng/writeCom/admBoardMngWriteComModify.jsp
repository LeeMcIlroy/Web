...
<script type="text/javaScript">
	...
	function fn_update(){
		...
		if(confirm(($("#brdSeq").val()==""? "등록":"수정") + "하시겠습니까?")){
			$("#frm").attr("method","post").attr("action", "<c:url value='/xabdmxgr/boardMng/writeCom/admBoardMngWriteComModify.do'/>").submit();
		}
	}
	
	// 목록으로 이동
	function fn_goList(){
		$("#frm").attr("method", "post").attr("action", "<c:url value='/xabdmxgr/boardMng/writeCom/admBoardMngWriteComList.do'/>").submit();
	}
...
<input type="hidden" id="searchType" name="searchType" value="WRITE"/>
		...
		<div class="content" id="content">
			...
			<div class="title_area">
				<jsp:include page="/WEB-INF/jsp/writer/adm/inc/incPageTitle.jsp">
		            ...
		            <jsp:param name="dept2" value="제15회 한성인 글쓰기대회"/>
            	</jsp:include>
			</div>
			<div class="cont_box">
			<!-- content -->
				...
				<table class="view_tbl_03 mb30" summary="제15회 한성인 글쓰기대회">
					<caption>제15회 한성인 글쓰기대회</caption>
					<colgroup>
						<col width="20%" />
						<col width="30%" />
						<col width="20%" />
					...
					<tbody>
						<tr class="first">
							...
							<td>
								<form:select path="brdType" class="se_base w100">
									<form:option value="WRITE">글쓰기</form:option>
									<form:option value="PPT">프레젠테이션</form:option>
								</form:select>
							</td>
							<th scope="row">* 개최년도</th>
							<td>
						...
						<tr>
							...
							<td colspan="3">
								<form:radiobutton path="useYn" value="Y" label="게시"/>&nbsp;&nbsp;&nbsp;
								<c:if test="${empty boardVO.brdSeq }">  
									<form:radiobutton path="useYn" value="N" label="미게시" checked="true"/>
								</c:if>
								<c:if test="${!empty boardVO.brdSeq }">
									<form:radiobutton path="useYn" value="N" label="미게시"/>
								</c:if>
							</td>
						</tr>
						<tr>
							<th scope="row">주제글 여부</th>
							<td colspan="3">
								<form:radiobutton path="topicYn" value="Y" label="주제글"/>&nbsp;&nbsp;&nbsp;
								<c:if test="${empty boardVO.brdSeq }">  
									<form:radiobutton path="topicYn" value="N" label="일반글" checked="true"/>
								</c:if>
								<c:if test="${!empty boardVO.brdSeq }">
									<form:radiobutton path="topicYn" value="N" label="일반글"/>
								</c:if>
							</td>
						</tr>
						<tr>
