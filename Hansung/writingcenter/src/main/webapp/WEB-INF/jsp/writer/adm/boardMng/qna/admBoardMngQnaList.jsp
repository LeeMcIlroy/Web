...
<script>
	...
	function fn_view(id){
		$("#qnaSeq").val(id);
		$("#frm").attr("method","post").attr("action","/xabdmxgr/boardMng/qna/admBoardMngQnaView.do").submit();
	}
						...
						<c:forEach var="list" items="${listVO }" varStatus="status">
							<tr>
								...
								<td style="text-align: left;">
									<a href="#" onclick="fn_view(<c:out value='${list.qnaSeq }'/>); return false;"><c:out value="${list.qstTitle }"/></a>
								</td>
							...
							<c:if test="${!empty list.ansTitle }">
								<tr>
									<td>답변</td>
									<td style="text-align: left;">
										<a href="#" onclick="fn_view(<c:out value='${list.qnaSeq }'/>); return false;">
											└<c:out value="${list.ansTitle }"/>
										</a>
									</td>
