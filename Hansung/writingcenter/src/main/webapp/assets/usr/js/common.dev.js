...
var gDatepickerOption4user = {
        showOn: 'button',
  		buttonImage: '/images/common/btn_calendar.gif', // 달력 아이콘 이미지 경로
...
var gDatepickerOption4admin = {
        showOn: 'button',
  		buttonImage: '/swer_mgr/images/board/btn_calendar.gif', // 달력 아이콘 이미지 경로
  		buttonImageOnly: true,
        buttonText: "달력",
        changeMonth: true,
  		changeYear: true,
...
function initControl(p, option) {
	...
	$(".datepicker", p).each(function(){
		...
		if (gUserType == "ADMIN") {
			datepickerOption = gDatepickerOption4admin;
		}
	...
	$('.numeric', p).numeric();
	$('.onlyEng', p).alphanumeric({allow:"!@#$%^&*()+=[]\\\';,/{}|\":<>?~`.-_ "});
