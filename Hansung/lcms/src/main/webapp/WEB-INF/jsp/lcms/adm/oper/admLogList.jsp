...
function fn_list(pageIndex){
	...
	$("#listForm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/qxsepmny/oper/admLogList.do'/>").submit();
}

//엑셀 다운로드
function fn_excelDown(){
	$("#listForm").attr("method", "post").attr("action", "<c:url value='/qxsepmny/oper/admLogExcel.do'/>").submit();
}
</script>
		...
		<div class="contents">
			...
			<div class="main-content">
				<jsp:include page="/WEB-INF/jsp/lcms/adm/inc/incPageTitle.jsp">
		            ...
		            <jsp:param name="dept2" value="개인정보처리(로그)"/>
	           	</jsp:include>
					...
					<table class="normal-list-table">
						...
						<c:forEach items="${resultList}" var="result" >
							<tr>
								...
								<td><c:out value="${result.regName}"/>(<c:out value="${result.regId }"/>)</td>
								<td><c:out value="${result.memberCode }"/></td>
								<td><c:out value="${result.name }"/></td>
								<td><c:out value="${result.eName }"/></td>
								<td><c:out value="${result.logCont }"/></td>
