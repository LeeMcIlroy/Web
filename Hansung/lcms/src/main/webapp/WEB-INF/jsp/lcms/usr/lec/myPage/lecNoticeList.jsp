...
<script>
		...
		function fn_list(pageNo){
			...
			$("#listForm").attr("action", "<c:url value='/usr/lec/myPage/lecNoticeList.do'/>").submit();
		}
		function rowClick(notiSeq){
			$("#noti_seq").val(notiSeq);
			$("#listForm").attr("action", "<c:url value='/usr/lec/myPage/lecNoticeView.do'/>").submit();
		}
		 function fn_close(){
		...
		function fn_lac(){
			$("#listForm").attr("action", "<c:url value='/usr/lec/myPage/lecProfileView.do'/>").submit();
		}
	</script>
<body>
						...
						<div class="search-input">
							<table class="shearch-option">
								...
								<tbody>
									<tr>
										<td>
											<input type="text" name="searchWord" value="${searchVO.searchWord }" class="input-data" placeholder="제목, 내용을 입력하세요" />
										</td>
									</tr>
								</tbody>
							</table>
					...
					<table class="normal-list-table">
						...
						<c:forEach items="${resultListTOP }" var="resultTop" varStatus="">
							<tr>
								...
								<td><c:out value="${resultTop.notiLechit }"/></td>
							</tr>
							</c:forEach>
							<!--************ 공지 리스트 20200312************-->
							...
							<tr>
								...
								<td><c:out value="${result.notiLechit }"/></td>
							</tr>
							</c:forEach>
						</tbody>
					</table>
					...
					<c:forEach items="${resultListTOP }" var="resultTop" varStatus="">
							<li>
								...
								<p class="option">
									...
									<span class="hit"><c:out value="${resultTop.notiLechit }"/></span>
								</p>
							</li>
						</c:forEach>
						<!--************ 모바일 공지 리스트 20200313************-->
						<c:forEach items="${resultList }" var="result" varStatus="">
							<li>
								...
								<p class="option">
									...
									<span class="hit"><c:out value="${result.notiLechit }"/></span>
								</p>
							</li>
						</c:forEach>
						</ul>
				...
				<div class="la-pop">
					...
					<div class="table-button">
						<div class="se-btn-box">
							...
							<button type="button" class="white btn-cancel" onclick="fn_close(); return false;">다음에</button>
						</div>
					</div>
				</div>
</form:form>
