...
<script>
		...
		function fn_modify(){ 
			...
			$("#detailForm").attr("action", "<c:url value= '/usr/std/myPage/stdProfileUpdate.do'/>").submit();
			alert('변경 되었습니다.');
		}
		...
		function fn_modify_pw(){
			...
			var spe = pw2.search(/[`~!@@#$%^&*|₩₩₩'₩";:₩/?]/gi);
			 ...
			 if(pw1 == ''){
				...
				$("#detailForm").attr("action", "<c:url value= '/usr/std/myPage/stdAjaxPasswordUpdate.do'/>").submit();
				alert('변경 되었습니다.');
			}
		...
		function fn_cancel(){
			$("#detailForm").attr("action", "<c:url value= '/usr/std/myPage/stdNoticeList.do'/>").submit();
			alert('취소 되었습니다.');
		}
		...
		function fn_pwChk(ele){
			...
			$.ajax({
				url: "<c:url value='/usr/std/myPage/admAjaxMemberPwChk.do'/>"
				, type: "post"
				...
				, success: function(data){
					...
					console.log(status);
					if(status){
						$("#pwChkTxt_2").removeClass('dpn');
						$("#pwChkTxt2_2").addClass('dpn');
						$("#pwChk_2").val('Y');
						...
						$("#pwChkTxt_2").addClass('dpn');
						$("#pwChkTxt2_2").removeClass('dpn');
						$("#pwChk2_2").val('N');
		...
		$(function(){
			...
			$("#alert-pw-guide_2").removeClass('dpn');
			$("#alert-danger_2").addClass('dpn');
			$("#alert-success_1_2").addClass('dpn');
			$("#alert-success_2_2").addClass('dpn');
			$("#alert-danger_1_2").addClass('dpn');
			$("#alert-pw-check_2").addClass('dpn');
			$("#alert-pw-check_1_2").addClass('dpn');
			...
			$("input").keyup(function(){
				...
				// 비밀번호 영문,숫자,특수문자 조합 8자리 이상 체크
				 ...
				 if(pw2.length < 8 || num < 0 || eng < 0 || spe < 0){
					...
					$("#alert-pw-check_2").removeClass('dpn');
					$("#alert-pw-guide_2").addClass('dpn');
					$("#alert-success_1_2").addClass('dpn');
					$("#alert-success_2_2").addClass('dpn');
					...
					return false;
					 ...
					 $("#alert-success_1_2").removeClass('dpn');
					 $("#alert-pw-check_2").addClass('dpn');
					 $("#alert-pw-guide_2").addClass('dpn');
				 }
				}
				...
				if(pw3 == ""){
					...
					if(pw2 != pw3){
						...
						$("#alert-danger_1_2").removeClass('dpn');
						$("#alert-pw-check_1_2").addClass('dpn');
						$("#alert-success_2_2").addClass('dpn');
						$("#submit").attr("disabled", "disabled");
						return false;
						...
						$("#alert-success_2_2").removeClass('dpn');
						$("#alert-danger_1_2").addClass('dpn');
						$("#alert-pw-check_2").addClass('dpn');
						$("#alert-pw-check_1_2").addClass('dpn');
						$("#alert-pw-guide_2").addClass('dpn');
							...
							<table class="normal-wmv-table">
								...
								<tbody>
									...
									<tr>
										<th>권한</th>
										<td>
											<c:out value="${lectSession.lectYear }"/>년도 <c:out value="${lectSession.lectSem eq '1'?'봄학기':lectSession.lectSem eq '2'?'여름학기':lectSession.lectSem eq '3'?'가을학기':'겨울학기' }"/> <c:out value="${lectSession.lectName }"/> <c:out value="${lectSession.lectDivi }"/>
										</td>
									</tr>
									<tr>
										<th>최초등록일시</th>
						...
						<!-- <div class="mob">
							<div class="mob-list">
								<div class="mob-list-type">
									...
									<dl>
										<dt>권한</dt>
										<dd>2019-봄학기-한국어1급-A반</dd>
									</dl>
									<dl>
										<dt>최초등록일시</dt>
										<dd>2019.10.10 18:30</dd>
									</dl>
								</div>
							</div>
						</div> -->
								...
								<tbody>
									...
									<tr>
										...
										<td>
											...
											<span class="alt-txt" id="alert-pw-guide">영문,숫자,특수문자 조합으로 8자 이상 설정</span>
											<span class="alt-txt" id="alert-pw-check">영문,숫자,특수문자 조합으로 8자 이상이어야 합니다.</span>
											<span class="ok-signature" id="alert-success_1">O.K</span>
										</td>
									</tr>
									<tr> 
										...
										<td>
											...
											<span class="alt-txt" id="alert-danger_1">비밀번호가 일치하지 않습니다.</span>
											<span class="alt-txt" id="alert-pw-check_1">영문,숫자,특수문자 조합으로 8자 이상이어야 합니다.</span>
							...
							<div class="mob-write w-dt130">
									...
									<dd>
										<ul>
											<li>
												...
												<span class="alt-txt dpn" id="pwChkTxt_2">비밀번호가 일치합니다.</span>
												<input type="hidden" id="pwChk_2" value="Y"/>
												<span class="alt-txt dpn" id="pwChkTxt2_2">비밀번호가 일치하지 않습니다.</span>
												<input type="hidden" id="pwChk2_2" value="N"/>
											</li>
										</ul>
									</dd>
								</dl>
								<dl>
									...
									<dd>
										...
										<span class="alt-txt" id="alert-pw-guide_2">영문,숫자,특수문자 조합으로 8자 이상 설정</span>
										<span class="alt-txt" id="alert-pw-check_2">영문,숫자,특수문자 조합으로 8자 이상이어야 합니다.</span>
										<span class="ok-signature" id="alert-success_1_2">O.K</span>
									</dd>
								</dl>
								<dl>
									...
									<dd>
										...
										<span class="alt-txt" id="alert-danger_1_2">비밀번호가 일치하지 않습니다.</span>
										<span class="alt-txt" id="alert-pw-check_1_2">영문,숫자,특수문자 조합으로 8자 이상이어야 합니다.</span>
										<span class="ok-signature dpn" id="alert-success_2_2">O.K</span>
									</dd>
								</dl>
								<dl>
									<dt>최종변경일</dt>
