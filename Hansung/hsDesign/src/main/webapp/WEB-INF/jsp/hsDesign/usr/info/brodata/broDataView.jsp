...
<script type="text/javascript">
	...
	function fn_select(cbSeq){
		...
		$("#frm").attr("method","get").attr("action", "<c:out value='${pageContext.request.contextPath }/usr/info/brodata/broDataView.do'/>").submit();
	}
	...
	function fn_select_add(){
		flag = false;
		$.ajax({
			url: "<c:out value='${pageContext.request.contextPath }/usr/info/brodata/broDataView.do'/>"
			, type: "get"
			...
			, success: function(data){
				...
				if(cmmBoardVO == null){
					...
					tags += "</ul>";
					tags += "<div class='tbl_cont'>"+cmmBoardVO.cbContent +"</div></div></div>";
					...
					if(!(recentList == null || recentList.length == 0)){
						...
						for(i =0; i<recentList.length; i++){
							...
							tags += "<dl><dt>"
							if(recentList[i].cbThType == 'IMG'){
								tags += "<img style='width:209px; height:129px;' src='<c:out value='/showImage.do?filePath="+recentList[i].cbThImgPath+"'/>' alt='"+recentList[i].cbThImgName+"' />"
										
							}else if(recentList[i].cbThType == 'AVI'){
		...
		<div class="width_box">
			...
			<div class="sub_content">
				...
				<jsp:include page="/WEB-INF/jsp/hsDesign/usr/inc/incPageTitle.jsp">
		            ...
		            <jsp:param name="dept2" value="작품자료실"/>
		            <jsp:param name="dept3" value="작품자료실"/>
	           	</jsp:include>
	           	<div id="viewPage">
					<div class="transform_table notice_type">
						<div class="tbl_view">
							<ul class="tbl_view_m title">
								<li class="txt_left">
									<c:if test="${!empty cmmBoardVO.cbGubun }">
										<div class="ticon_type01" style="float: left;">
											<c:out value="${cmmBoardVO.cbGubun }"/>	
										</div> 
									</c:if>
									<c:out value="${cmmBoardVO.cbTitle }" escapeXml="false"/>
								</li>
