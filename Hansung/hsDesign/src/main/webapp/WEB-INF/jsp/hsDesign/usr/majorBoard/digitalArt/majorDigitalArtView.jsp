...
<script type="text/javascript">
	...
	// 목록
	function fn_list(){
		$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/usr/majorBoard/digitalArt/majorDigitalArtList.do'/>").submit();
	}
	
	// 조회
	function fn_select(mbSeq){
		$("#mbSeq").val(mbSeq);
		$("#frm").attr("method", "get").attr("action", "<c:out value='${pageContext.request.contextPath }/usr/majorBoard/digitalArt/majorDigitalArtView.do'/>").submit();
	}
	...
	function fn_select_add(){
		flag = false;
		$.ajax({
			url: "<c:out value='${pageContext.request.contextPath }/usr/majorBoard/digitalArt/majorDigitalArtPreView.do'/>"
			, type: "get"
					...
					for(i =0; i<recentList.length; i++){
						...
						if(recentList[i].mbthType == 'IMAGE'){
							if(recentList[i].mbOldYn == 'Y'){
								tags += "<img style='width:209px; height:129px;' src='<c:out value='/showOldImage.do?filePath="+recentlist[i].mbthImgPath+"'/>' alt='"+recentList[i].mbthImgName +"'/>";
							}else{
								tags += "<img style='width:209px; height:129px;' src='<c:out value='/showImage.do?filePath="+recentList[i].mbthImgPath+"'/>' alt='"+recentList[i].mbthImgName+"' />";
							...
							tags += "<img style='width:209px; height:129px;' src='<c:out value='/assets/usr/img/no_img.jpg'/>' alt='한디원' />";
						}
...
<input type="hidden" id="menuNo" name="menuNo" value="<c:out value="${sessionScope.menuNo}"/>">
							...
							<dl>
								<dt>
									...
									<c:if test="${list.mbthType eq '' || list.mbthType eq null}">
										<img  src="<c:out value='/assets/usr/img/no_img.jpg'/>" alt="한디원" />
									</c:if>
								</dt>
