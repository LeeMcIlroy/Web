...
<script type="text/javascript">
	...
	function fn_select_pwd(){
		$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/usr/general/question/generalQuestionView.do'/>").submit();
	}
					...
					<div class="tbl_td">
						<c:forEach items="${ntResultList }" var="ntResult" varStatus="status">
							<ul>
								...
								<li>
								<c:if test="${!empty ntResult.regName }">
									<c:out value="${ntResult.regName }" />
								</c:if> <c:if test="${empty ntResult.regName }">&nbsp;</c:if>
								</li>
