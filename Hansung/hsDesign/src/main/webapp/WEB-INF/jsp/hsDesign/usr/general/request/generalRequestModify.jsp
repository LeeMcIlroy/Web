...
<script type="text/javascript">
	...
	function fn_update(){
		...
		$("#frm").attr("method","post").attr("action", "<c:out value='${pageContext.request.contextPath }/usr/general/request/generalRequestUpdate.do'/>").submit();
	}
					...
					<div class="tbl_write">
						...
						<c:if test="${!empty cmmBoardVO.cbSeq }"> <!--게시글 수정 게시판의 번호가 기존에 있다면  -->
							...
							<input type="hidden" name="regName" value="${cmmBoardVO.regName }" />
                                                        <input type="hidden" name="cbPassword" value="${cmmBoardVO.cbPassword }" />						</c:if> 
