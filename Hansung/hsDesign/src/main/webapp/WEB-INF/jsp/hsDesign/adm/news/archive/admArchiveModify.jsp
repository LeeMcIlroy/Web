				...
				<!-- content -->
					...
					<table class="view_tbl_03 mb30" summary="작품자료실">
						...
						<tbody>
							...
							<td>
								<form:select path="cbGubun" class="se_base w150" onchange="fn_change(); return false;">
									<form:option value="">선택</form:option>
									<c:forEach var="list" items="${majorList }">
										<c:if test="${list.mCode lt 6 || list.mCode eq 7 || list.mCode eq 8}">
											<option value="${list.mName }" <c:if test="${list.mName eq cmmBoardVO.cbGubun}">selected = "selected"</c:if>><c:out value="${list.mName }"/></option>
										</c:if>
									</c:forEach>
								</form:select>
							</td>
