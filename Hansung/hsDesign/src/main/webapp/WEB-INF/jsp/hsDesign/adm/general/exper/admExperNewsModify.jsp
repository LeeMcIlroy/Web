...
<script>
	...
	<c:if test="${empty majorBoardVO.mbSeq}">
		...
		$("#frm").attr("method","post").attr("action", "<c:out value='${pageContext.request.contextPath }/qxerpynm/general/exper/admExperNewsUpdate.do'/>").submit();
	}
	
	// 목록으로 이동
	function fn_list(){
		$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/qxerpynm/general/exper/admExperNewsList.do'/>").submit();
	}
		...
		<div class="content" id="content">
			...
			<div class="title_area">
				<jsp:include page="/WEB-INF/jsp/hsDesign/adm/inc/incPageTitle.jsp">
		            ...
		            <jsp:param name="dept2" value="진로체험소식"/>
            	</jsp:include>
			</div>
			<div class="cont_box">
			<!-- content -->
				...
				<form:hidden path="mbGubun1" value="52"/>
				<table class="view_tbl_03 mb30" summary="실내디자인">
					<caption>실내디자인</caption>
					<colgroup>
						<col width="20%" />
						<col width="80%" />
					</colgroup>
					<tbody>
						<%-- <tr class="first">
							...
							<td>
								...
								<c:if test="${empty majorBoardVO.mbSeq }">
									<form:select path="mbGubun2" class="se_base w200">
										<form:option value="">선택</form:option>
									</form:select>
								</c:if>
								<c:if test="${!empty majorBoardVO.mbSeq }">
						...
						<tr>
							...
							<td>
								...
								<ul class="file_up">
									...
									<li>
										<form:select path="mbthType" class="se_base w100">
									...
									<li id="imageUpfile" >
										<c:if test="${!empty majorBoardVO.mbthImgName }">
					        				<label for="fileDeleteChk"><c:out value="${majorBoardVO.mbthImgName  }"/></label>
                               				<a href="#" onclick="fn_thumb_delete(); return false;" style="display:inline-block; color:#fff; background-color:#000; height:15px; font-size:10px; padding:0 5px; line-height:15px;">X</a>
										</c:if>
										<c:if test="${empty majorBoardVO.mbthImgName }">
											<input type="file" id="titleImage" name="titleImage" class="file"/>
										</c:if>
									</li>
									<li id="videoUrl">
										<form:input path="mbthUrl" class="in_base" style="width:400px; height:25px; margin-left:5px;" />
									</li>
								</ul>
								 --%>
								<ul class="file_up">
								...
								<!-- <div id="imageTxt" class="alt_txt" style="margin-left:197px;" >JPG, GIF 형식으로 입력해 주세요</div> -->
								<!-- <div id="videoTxt" class="alt_txt" style="margin-left:109px; padding-top:5px;" >YouTube 또는 Vimeo에서 제공하는 공유 주소를 입력해 주세요.</div> -->
							</td>
						</tr>
						<tr>
