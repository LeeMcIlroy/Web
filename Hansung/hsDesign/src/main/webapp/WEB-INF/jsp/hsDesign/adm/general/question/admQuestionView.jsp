...
<c:if test="${paginationInfo.totalRecordCount eq 0 }">	
	...
	function fn_content_reply(cbSeq){
		if($.trim($("#cbContentReply").val()) == ''){
			alert("답변 내용을 입력해주세요.");
			return false;
		}
		$("#cbSeq").val(cbSeq);
		$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath}/qxerpynm/cmmBoard/admCmmBoardContentReply.do'/>").submit();
	}
//  답글 작성 끝*****************************************************************	
		...
		<div class="container">
			...
			<div class="content" id="content">
				...
				<c:import url="/EgovPageLink.do?link=adm/inc/incPageTitle">
					...
					<c:param name="dept2" value="교양개설문의"/>
				</c:import>
				<div class="cont_box">
				<!-- content -->
					<!-- table -->
					<table class="view_tbl_03 mb30 mt30" summary="교양개설문의">
						<caption>교양개설문의</caption>
						<colgroup>
							<col width="13%" />
							<col width="20%" />
							<col width="13%" />
				...
				<c:if test="${!empty cmmBoardVO.cbContentReply}">
					<table class="view_tbl_03 mb30 mt30" id="contentReply" >
						<colgroup>
							<col width="13%" />
							<col width="20%" />
							<col width="13%" />
						...
						<tbody>
							<tr class="first">
								<th scope="row" colspan="6"><img src="<c:out value='${pageContext.request.contextPath }/assets/adm/img/Re.png'/>" />
									Re : <c:out value="${cmmBoardVO.cbTitle }" escapeXml="false"/></a></th>
							</tr>
							<tr>
								<th scope="row">작성자</th>
								<td><c:out value="${cmmBoardVO.cbConReplyName }"/></td>
								<th scope="row">작성일</th>
								<td><c:out value="${cmmBoardVO.cbConReplyDate }"/></td>
								<th scope="row">조회수</th>
								<td><c:out value="${cmmBoardVO.cbCount }"/></td>
							</tr>
						</tbody>
					...
					<div class="coment_write">
						<div class="cmt_int">
							<textarea id="cbContentReply" name="cbContentReply" cols="5" rows="5">${cmmBoardVO.cbContentReply}</textarea>
							<c:if test="${cmmBoardVO.cbContentReply == null}">
							<a href="#" onclick="fn_content_reply(<c:out value='${cmmBoardVO.cbSeq }'/>); return false;">답글등록</a>
						</c:if>
						<c:if test="${cmmBoardVO.cbContentReply != null}">
						<a href="#" onclick="fn_content_reply(<c:out value='${cmmBoardVO.cbSeq }'/>); return false;">답글수정</a>
						</c:if>
						</div>
						</div>
