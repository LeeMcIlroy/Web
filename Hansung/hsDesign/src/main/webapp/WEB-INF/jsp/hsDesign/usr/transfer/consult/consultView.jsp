...
<script type="text/javascript">
	...
	function fn_delete(){
		if(confirm("삭제하시겠습니까?")){
			$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/usr/transfer/consult/consultDelete.do'/>").submit();
		}
	}
	// 댓글 입력
	function fn_comment_update(){
		...
		$.ajax({
			url: "<c:out value='${pageContext.request.contextPath }/usr/transfer/consult/consultCommentUpdate.do'/>"
			, type: "post"
			, data: $("#frm").serialize()
			, dataType:"json"
	...
	function fn_comment_delete(cbcoSeq){
		if(confirm("삭제하시겠습니까?")){
			...
			$.ajax({
				url: "<c:out value='${pageContext.request.contextPath }/usr/transfer/consult/consultCommentDelete.do'/>"
				, type: "post"
				, data: $("#frm").serialize()
				, dataType:"json"
				, success: function(data){
		...
		<div class="width_box">
			...
			<div class="sub_content">
				...
				<jsp:include page="/WEB-INF/jsp/hsDesign/usr/inc/incPageTitle.jsp">
					<jsp:param name="dept1" value="편입"/>
		            <jsp:param name="dept2" value="편입상담"/>
	           	</jsp:include>
