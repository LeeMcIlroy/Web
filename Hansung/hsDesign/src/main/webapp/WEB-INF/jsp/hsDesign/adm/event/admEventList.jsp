...
<script type="text/javascript">
	...
	// 행사비 입금여부 수정
	function fn_modify_depoYn(eveSeq, depoYn){
		$("#eveSeq").val(eveSeq);
		...
		<div class="container">
			...
			<div class="content" id="content">
				...
				<c:import url="/EgovPageLink.do?link=adm/inc/incPageTitle">
					...
					<c:param name="dept2" value="행사참가신청"/>
				</c:import>
				<div class="cont_box">
				<!-- content -->
							...
							<ul>
								<li>
									<form:select path="searchType" class="se_base w100">
										<form:option value="">입금여부</form:option>
					...
					<table class="list_tbl_03" summary="행사참가신청 목록">
						...
						<tbody>
							<c:forEach items="${resultList }" var="result" varStatus="status">
								<tr>
									...
									<td><c:out value="${result.eveDepoDate }"/></td>
									<c:if test="${result.eveDepoYn == 'Y' }">
									<td class="bg_yes" >YES</td>
									<td onclick="fn_modify_depoYn('<c:out value="${result.eveSeq}"/>' ,'N'); return false;" style="cursor: pointer;">NO</td>
									</c:if>
									<c:if test="${result.eveDepoYn == 'N' }">
									<td onclick="fn_modify_depoYn('<c:out value="${result.eveSeq}"/>' ,'Y'); return false;" style="cursor: pointer;">YES</td>
