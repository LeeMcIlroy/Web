...
<script type="text/javascript">
	...
	function fn_list3(pageNo, searchCondition2){
		$("#pageIndex").val(pageNo);
		$("#searchCondition2").val(searchCondition2);
		$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/qxerpynm/elite/teacher/admEliteTeacherList.do'/>").submit();
	}
	...
	function fn_modify(tcSeq){
		$("#tcSeq").val(tcSeq);
		$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/qxerpynm/elite/teacher/admEliteTeacherModify.do'/>").submit();
	}
	...
	function fn_delete(tcSeq){
		if(confirm("삭제 하시겠습니까?")){
			$("#tcSeq").val(tcSeq);
			$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/qxerpynm/elite/teacher/admEliteTeacherDelete.do'/>").submit();
		}
	}
			...
			<!-- content -->
				<div class="mid_tab01 mt20">
					<ul>
						...
						<%-- <c:forEach var="list" items="${majorList }">
							<c:if test="${list.mCode > 10  }">
								<li><a style="cursor: pointer;" <c:if test="${fn:split(list.mName,'-')[1] eq searchVO.searchCondition2 }">class="active"</c:if> onclick="fn_list3(1,'${fn:split(list.mName,'-')[1]}'); return false;"><c:out value="${list.mName }"/></a></li>
								<li><a style="cursor: pointer;" <c:if test="${list.mName eq searchVO.searchCondition2 }">class="active"</c:if> onclick="fn_list3(1,'<c:out value='${list.mName }'/>'); return false;"><c:out value="${list.mName }"/></a></li>
							</c:if>
						</c:forEach> --%>
					</ul>
				</div>
								...
								<%-- 200421추가/수정 --%>
									<c:choose>
										<%-- <c:when test="${searchVO.searchCondition1 eq '12' }">
											<form:option value='성우콘텐츠 크리에이터'>성우콘텐츠 크리에이터</form:option>
										</c:when>
										<c:when test="${searchVO.searchCondition1 eq '13' }">
											<form:option value='글로벌 패션 전자상거래 전문가'>글로벌 패션 전자상거래 전문가</form:option>
	
										</c:when>
										<c:when test="${searchVO.searchCondition1 eq '14' }">
											<form:option value='헤어디자이너'>헤어디자이너 </form:option>			
										</c:when>
	 									<c:otherwise>  
											<form:option value='타일아트 시공 전문가'>타일아트 시공 전문가</form:option>");
										</c:otherwise>--%>
										<c:when test="${searchVO.searchCondition1 eq '14' }">
											<form:option value='뷰티교육자'>뷰티교육자 </form:option>	
											<form:option value='하야시두피모발'>하야시두피모발 </form:option>				
										</c:when>
										<c:otherwise>
											<form:option value='하야시두피모발'>하야시두피모발 </form:option>			
										</c:otherwise>
