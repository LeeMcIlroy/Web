...
<script type="text/javascript">
	...
	// 등록 & 수정 화면
	function fn_modify(){
		$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/usr/general/question/generalQuestionModify.do'/>").submit();
	}
	
	// 삭제 
	function fn_delete(){
		if(confirm("삭제하시겠습니까?")){
			$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/usr/general/question/generalQuestionDelete.do'/>").submit();
		}
	}
	// 댓글 입력
	function fn_comment_update(){
		...
		$.ajax({
			url: "<c:out value='${pageContext.request.contextPath }/usr/general/question/generalQuestionCommentUpdate.do'/>"
			, type: "post"
			, data: $("#frm").serialize()
			, dataType:"json"
			, success: function(data){
				...
				alert("댓글이 등록되었습니다.");
				$('.btn_go_mody').attr('style','display:none');
				$('.btn_go_del').attr('style','display:none');
	...
	function fn_comment_delete(cbcoSeq){
		...
		if(confirm("삭제하시겠습니까?")){
			...
			$.ajax({
				url: "<c:out value='${pageContext.request.contextPath }/usr/general/question/generalQuestionCommentDelete.do'/>"
				, type: "post"
				, data: $("#frm").serialize()
				, dataType:"json"
				, success: function(data){
					...
					if ( $("#commentList div").length > 0 ) {
						$('.btn_go_mody').attr('style','display:none');
						$('.btn_go_del').attr('style','display:none');
					}else{
						$('.btn_go_mody').attr('style','visibility:visible');
						$('.btn_go_del').attr('style','visibility:visible');
						//공간의 밀림없이 보이게 하기 위해서
					}
...
<!-- ***************************	답변 추가  ***************************-->
                    ...
                    <c:if test="${!empty cmmBoardVO.cbContentReply && cmmBoardVO.cbNoticeYn.equals('N')}">
					<div class="tbl_view" style="margin-top: 2px;">
						<ul class="tbl_view_m">
							<li class="txt_left"><img src="<c:out value='${pageContext.request.contextPath }/assets/adm/img/Re.png'/>" />
									Re : <c:out value="${cmmBoardVO.cbTitle }" escapeXml="false"/></li>
							<li>작성자 : <c:out value="${cmmBoardVO.cbConReplyName }"/></li>
							<li><c:out value="${cmmBoardVO.cbConReplyDate }"/></li>
							<li><c:out value="${cmmBoardVO.cbCount }"/></li>
						</ul>
						<div class="tbl_cont">
							<c:out value="${cmmBoardVO.cbContentReply }" escapeXml="false"/>
						</div>
						</div>
						</c:if>
<!-- ***************************	답변 추가  ***************************-->
				...
				<div class="btn_box" >
					<c:if test="${cmmBoardVO.cbNoticeYn.equals('N')}">
						<c:if test="${cmmBoardVO.cbContentReply == null }">
					...
					<div class="comment">
						...
						<c:forEach var="list" items="${cbCommentList }">
							<dl>
								...
								<c:if test="${empty list.cbcoRegSeq}">| <a href="#" onclick="fn_comment_delete(<c:out value="${list.cbcoSeq }"/>); return false;">삭제</a></c:if></span></dt>
								<dd>
