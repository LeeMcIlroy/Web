...
<script type="text/javascript">
	...
	function fn_select(wSeq){
		$("#wSeq").val(wSeq);
		$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/qxerpynm/webtoon/admWebtoonView.do'/>").submit();
	}
	...
	// 웹툰관리 목록
	function fn_webtoon_list(){
		/*
		var url = "<c:out value='${pageContext.request.contextPath}/qxerpynm/webtoon/admWebtoonCategoryList.do'/>";
		var option = "";
		window.open(url, "webtoonCategoryList", option);
		*/
		$("#menuType").val("MNG");
				...
				<!-- content -->
					<div class="tbl_top_side">
						<div class="page_btn" style="position: relative;">
							<a href="#" onclick="fn_webtoon_list(); return false;" class="b_type02">한툰 관리</a>
						</div>
						<div class="side_r">
							<ul>
								<li>
								...
								<li>
									<form:select path="searchCondition2" class="se_base w100">
										<form:option value="title">화 제목</form:option>
						...
						<tbody>
							<c:forEach items="${resultList }" var="result" varStatus="status">
								<tr>
									...
									<td class="ta_left">
										<a href="#" onclick="fn_select(<c:out value='${result.wSeq }'/>); return false;">
											<c:out value="${result.wcTitle }" escapeXml="false"/>
										</a> 
									</td>
									<td class="ta_left"><c:out value="${result.wTitle }" escapeXml="false"/></td>
									<td><c:out value="${result.regName }"/></td>
									<td><c:out value="${result.regDate }"/></td>
									<td><c:out value="${result.wCount }"/></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
					<div class="btm_area">
						...
						<div class="page_btn">
							<a href="#" onclick="fn_modify(); return false;" class="b_type03">한툰 회차 등록</a>
						</div>
					</div>
				<!-- //content -->
				</div>
