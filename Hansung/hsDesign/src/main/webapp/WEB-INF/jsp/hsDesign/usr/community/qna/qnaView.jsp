...
<script type="text/javascript">
	...
	// 목록
	function fn_list(){
		$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/usr/community/qna/qnaList.do'/>").submit();
	}

	// 등록 & 수정 화면
	function fn_modify(){
		$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/usr/community/qna/qnaModify.do'/>").submit();
	}
	
	// 삭제 
	function fn_delete(){
		if(confirm("삭제하시겠습니까?")){
			$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/usr/community/qna/qnaDelete.do'/>").submit();
		}
	}
	// 댓글 입력
	function fn_comment_update(){
		...
		$.ajax({
			url: "<c:out value='${pageContext.request.contextPath }/usr/community/qna/qnaCommentUpdate.do'/>"
			, type: "post"
			, data: $("#frm").serialize()
			, dataType:"json"
	...
	function fn_comment_delete(cbcoSeq){
		if(confirm("삭제하시겠습니까?")){
			...
			$.ajax({
				url: "<c:out value='${pageContext.request.contextPath }/usr/community/qna/qnaCommentDelete.do'/>"
				, type: "post"
				, data: $("#frm").serialize()
				, dataType:"json"
				, success: function(data){
