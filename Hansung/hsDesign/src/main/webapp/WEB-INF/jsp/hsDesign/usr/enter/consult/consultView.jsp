...
<script type="text/javascript">
	...
	// 목록
	function fn_list(){
		$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/usr/enter/consult/consultList.do'/>").submit();
	}

	// 등록 & 수정 화면
	function fn_modify(){
		$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/usr/enter/consult/consultModify.do'/>").submit();
	}
	
	// 삭제 
	function fn_delete(){
		if(confirm("삭제하시겠습니까?")){
			$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath }/usr/enter/consult/consultDelete.do'/>").submit();
		}
	}
	// 댓글 입력
	function fn_comment_update(){
		...
		$.ajax({
			url: "<c:out value='${pageContext.request.contextPath }/usr/enter/consult/consultCommentUpdate.do'/>"
			, type: "post"
			, data: $("#frm").serialize()
			, dataType:"json"
	...
	function fn_comment_delete(cbcoSeq){
		if(confirm("삭제하시겠습니까?")){
			...
			$.ajax({
				url: "<c:out value='${pageContext.request.contextPath }/usr/enter/consult/consultCommentDelete.do'/>"
				, type: "post"
				, data: $("#frm").serialize()
				, dataType:"json"
				, success: function(data){
		...
		<div class="width_box">
			...
			<div class="sub_content">
				...
				<jsp:include page="/WEB-INF/jsp/hsDesign/usr/inc/incPageTitle.jsp">
		            ...
		            <jsp:param name="dept2" value="입학상담"/>
	           	</jsp:include>
				...
				<c:if test="${cmmBoardVO.regSeq eq 0 }">
					<div class="comment">
						...
						<c:forEach var="list" items="${cbCommentList }">
							<dl>
								<dt><c:out value="${list.cbcoRegName }"/> <span><c:out value="${list.cbcoRegDate }"/></span> 
									<span class="comment_cont">
										<c:if test="${empty list.cbcoRegSeq}"><%-- <a href="#" onclick="fn_comment_update(<c:out value="${list.cbcoSeq }"/>); return false;">수정</a>  --%>| <a href="#" onclick="fn_comment_delete(<c:out value="${list.cbcoSeq }"/>); return false;">삭제</a></c:if>
									</span>
								</dt>
								<dd>
