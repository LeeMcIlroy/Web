...
@Controller
public class Ech0801Controller extends AdmCmmController{
	
	private static final Logger LOGGER = LoggerFactory.getLogger(Ech0801Controller.class);
	@Autowired private Ech0801DAO ech0801DAO;
	...
	@RequestMapping("/qxsepmny/ech0801/ech0801List.do")	
	public String ech0801List(HttpServletRequest request, ModelMap model, @ModelAttribute("searchVO") CmmnSearchVO searchVO) throws Exception {
		LOGGER.debug("/qxsepmny/ech0801/ech0801List.do - 운영관리 로그인접속 목록화면");
		...
		CmmnListVO listVO = ech0801DAO.selectEch0801List(searchVO); 
		...
		return "/adm/ech0801/ech0801List";
	}

	// 로그인접속 리스트로 리다이렉트
	private String redirectList(RedirectAttributes reda, String message) {
		reda.addFlashAttribute("message", message);
		return "redirect:/qxsepmny/ech0801/ech0801List.do";
	}

	// 로그인접속 목록 엑셀 다운로드
	@SuppressWarnings("unchecked")
	@RequestMapping("/qxsepmny/ech0801/ech0801Excel.do")
	public void ech0801Excel(@ModelAttribute("searchVO") CmmnSearchVO searchVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		LOGGER.debug("/qxsepmny/ech0801/ech0801Excel.do - 로그인접속 목록 엑셀 다운로드");
		LOGGER.debug("searchVO = " + searchVO.toString());
		EgovMap dataMap = new EgovMap();
		
		List<EgovMap> resultList = ech0801DAO.selectEch0801Excel(searchVO);
		...
		excelUtil.getPerformanceExcel(dataMap, "로그인접속 리스트", "ech0801", request, response);
	}
