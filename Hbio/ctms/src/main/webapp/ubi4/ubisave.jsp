...
//	String savePath = appPath + "/ubi4/storage/" + TODAY;
	String savePath = "D:/WAS_JAVA/FactCTMS_HNB/Files/upload/attach/";
	...
	try {
		...
		if(type.indexOf("agree") != -1){
			...
			if(type.indexOf("crf") != -1) {
				//saveFileName = "CRF_"  +  paramMap.get("usrName") +  "_" +  System.currentTimeMillis() + ".pdf";
				saveFileName = "CRF_"  + paramMap.get("setRsNo") +  "_" +  System.currentTimeMillis() + ".pdf";
			}else {
				saveFileName = "설문_" + paramMap.get("setRsNo") +  "_" +  System.currentTimeMillis() + ".pdf";
			}
		}
		// PDF 파일명 ( 고객코드_파일명_시간 로 생성 )
		...
		if (columnNames != null) {
			for (i = 0; i < columnNames.length; i++) {
				...
				// 체크박스 값 - 20210330 수정
				else if( inputName.indexOf("CB") > 0 ) {
					// CheckBox 값은 "[checked]" 형태로 받음
					sb.append("indexOfCB--1----------------------------------").append(NL);
					sb.append(inputName.indexOf("CB")).append(NL);
					System.out.println(sb.toString());
					sb.delete(0, sb.length());
					
					String tmp = ubiEformData.getParameter(inputName);
					
					sb.append("tmp--1----------------------------------").append(NL);
					sb.append(ubiEformData.getParameter(inputName)).append(NL);
					...
					if(!tmp.isEmpty()) {
						...
						sb.append("tmp--1-2----------------------------------").append(NL);
						sb.append(tmp).append(NL);
						System.out.println(sb.toString());
						
						if("checked".equals(tmp)){
							String answNum = nmSpl[1].substring(2);
							
							sb.append("answNum--1----------------------------------").append(NL);
							sb.append(answNum).append(NL);
							System.out.println(sb.toString());
							
							map.put("answNum", Integer.parseInt(answNum) + 1);
							map.put("qtype", "C");
						}
					}
				}
				// 라디오버튼 이외의 값
					...
					sb.append("indexOfCB--2----------------------------------").append(NL);
					sb.append(inputName.indexOf("CB")).append(NL);
					System.out.println(sb.toString());
					
					sb.append("tmp--2----------------------------------").append(NL);
					sb.append(ubiEformData.getParameter(inputName)).append(NL);
					System.out.println(sb.toString());
					sb.delete(0, sb.length());
