<%@ page contentType="text/html; charset=utf-8" pageEncoding="utf-8" %>
<%@ page import="com.ubireport.common.util.StrUtil" %>
<%
	// 보안을 위해 설치 후 임시로 false로 변경해서 결과 확인 후 소스 배포 시 무조건 true로 변경해야함
	boolean refererCheck = false;
	String referer = StrUtil.nvl(request.getHeader("referer"), "");	// REFERER 가져오기
	if( refererCheck && (referer.equals("") || referer.indexOf(request.getServerName()) == -1) ) { 	// REFERER 체크(브라우저에서 직접 호출 방지)
		out.clear();
		out.print("비정상적인 접근입니다.");
		return;
	}
	//웹어플리케이션명
	String appName = StrUtil.nvl(request.getContextPath(), "");
	if( "/".equals(appName) )
		appName = "";
	//웹어플리케이션 Root URL, ex)http://localhost:8080/myapp
	String appUrl = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + appName;

	//UI에서 호출될 때 필요한 정보
	String file = StrUtil.nvl(request.getParameter("file"), "ubi_samp.jrf");

//	String arg = StrUtil.encrypt64(StrUtil.nvl(request.getParameter("arg"), "user#홍길동#company#유비디시전#"),"UTF-8");
	String arg = StrUtil.nvl(request.getParameter("arg"), "");
	System.out.println("[arg]_맨처음 " + arg);
//	arg += "#C_CODE#1234";
	arg = StrUtil.encrypt64(arg, "UTF-8");
	String resid ="";
	if(arg ==null || arg ==""){
		
	 resid = StrUtil.nvl(request.getParameter("resid"), "UBIHTML2");
	}else{
	 resid = StrUtil.nvl(request.getParameter("resid"), "UBIHTML");
		
	}

	String exportNm = StrUtil.nvl(request.getParameter("exportNm"), "HBSS-MPI-20003-8_결과보고서_에빅스젠");

	System.out.println("[appUrl] " + appUrl);
	System.out.println("[file] " + file);
	System.out.println("[arg] " + arg);
	System.out.println("[resid] " + resid);

	//추가
	System.out.println("[exportNm] " + exportNm);

	out.clearBuffer();
%>
<!DOCTYPE html PUBLIC '-//W3C//DTD HTML 4.01 Transitional//EN' 'http://www.w3.org/TR/html4/loose.dtd'>
<html>
<head>
<meta http-equiv="X-UA-Compatible" content="IE=edge"/>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0,minimum-scale=1.0,maximum-scale=1.5,user-scalable=yes">
<title>UbiReport Viewer</title>
<link rel="stylesheet" type="text/css" href="./css/ubieform.css" />
<!--[if IE]><script src='./js/ubiexcanvas.js'></script><![endif]-->
<script src='./js/ubicommon.js'></script>
<script src='./js/ubihtml.js'></script>
<script src='./js/msg.js'></script>
<script src='./js/ubinonax.js'></script>
<script src='./js/ubieform.js'></script>

<script language='javascript'>
<!--
var ubiHtmlViewer = null;
var ubiParams = {
	"appname": "<%= appName %>"
	,"ubikey": "<%= session.getId() %>" 
	,"ubiserver": "/UbiServer"
	,"ubisave": "/ubi4/ubisave.jsp"
	,"ubiresource": "/ubi4/js"
	,"ubiscale": "PageWidth"	//WholePage
	,"ubifile": "<%= file %>"
	,"ubiarg": "<%= arg %>"
	,"ubiresid": "<%= resid %>"
	,"reporttitle" : "<%= exportNm %>"

		,"reportevent.previewend": ubiReportPreviwEnd
		,"reportevent.printend": ubiReportPrintEnd
		,"reportevent.exportend": ubiReportExportEnd
		,"eformevent.previewend": ubiEformPreviewEnd
		,"eformevent.saveend": ubiEformSaveEnd
};

function ubiStart() {
	ubiHtmlViewer = UbiLoad(ubiParams);
	//sessionStorage.removeItem("totalpage",totalpage);

	try { /*ubiHtmlViewer.setUserSaveList('Image,Pdf,Docx,Xls,Pptx,Hml,Cell');*/ }catch(e){}
	try { /*ubiHtmlViewer.setUserPrintList('Ubi,Html,Pdf');*/ }catch(e){}
	try { /*ubiHtmlViewer.setVisibleToolbar('INFO', false);*/ }catch(e){}
	try { /*ubiHtmlViewer.setVisibleToolbar("SAVE", false);*/ }catch(e){}
}

function ubiReportPreviwEnd() {
	console.log('ubiPreviwEnd......');

	//	   htmlViewer.setVisibleToolbar("SAVE_PDF",false);
	   __ubiViewer.setVisibleToolbar("SAVE_EXCEL",false);
	   __ubiViewer.setVisibleToolbar("SAVE_HML",false);
//	   htmlViewer.setVisibleToolbar("SAVE_DOCX",false);
	   __ubiViewer.setVisibleToolbar("SAVE_PPTX",false);
	   __ubiViewer.setVisibleToolbar("SAVE_CELL",false);
	   __ubiViewer.setVisibleToolbar("SAVE_IMAGE",false);
	   __ubiViewer.setVisibleToolbar("PRINT_UBI",false);
	   __ubiViewer.setVisibleToolbar("PRINT_HTML",false);
	   __ubiViewer.setPrintMenu('PDF');
	   

}

function ubiReportPrintEnd() {
	console.log('ubiReportPrintEnd......');
}

function ubiReportExportEnd() {
	console.log('ubiExportEnd......');
}

function ubiEformSaveEnd() {
	console.log('ubiEformSaveEnd......');
}

function ubiEformPreviewEnd() {
	console.log('ubiEformPreviewEnd......');
	
	var totalpage = ubiHtmlViewer.totalPage;
	opener.sessionStorage.setItem("totalpage",totalpage);
	sessionStorage.setItem("totalpage",totalpage);

	
	console.log('totalpage : ' + totalpage);
	 
};

function ubiEformSaveEnd(file) {
	console.log('ubiEformSaveEnd......' + file);
};

//-->
</script>
</head>
<body onload='ubiStart()'>

</body>
</html>
