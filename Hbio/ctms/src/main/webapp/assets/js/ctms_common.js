	...
	if (id == "titleImg") {
		...
		&& fileExt != "PPT" && fileExt != "XLS" && fileExt != "PDF"
			&& fileExt != "BMP" && fileExt != "JPG" && fileExt != "GIF" 
				&& fileExt != "PNG"  && fileExt != "XLSX" && fileExt != "AI" && fileExt != "JEF" && fileExt != "JRF") {
...
function fn_search(ele){
	$.ajax({
		url: '/qxsepmny/cmm/CmmAjaxSearchYearRsCdList.do'
		, type: "post"
		...
		, success: function(data){
			...
			for(var i=0; i<resultList.length; i++){
				html += '<option value="'+resultList[i].rsCd+'">'+resultList[i].rsCd+'</option>';
			}
...
function fn_search_rsplan(ele){
	$.ajax({
		url: '/qxsepmny/cmm/CmmAjaxSearchYearRsCdList2.do'
		, type: "post"
		, data: "searchYear="+$("#searchYear").val()
		, dataType:"json"
		, success: function(data){
...
function fn_search_rsProtocol(ele){
	var rsNo2 = $("#rsNo2").val();
	$.ajax({
		url: '/qxsepmny/cmm/CmmAjaxSearchRsProtocolList.do'
		, type: "post"
		, data: "rsNo2="+$("#rsNo2").val()
		, dataType:"json"
		, success: function(data){
			resultList = data.resultList;
			var html = "";
...
function fn_ubi_pop(frmName, type, argVal, exportNm, tempNo) {
	var frm = document.getElementById(frmName);
	//alert(argVal);
	//console.log(frm);
	var winObj = window.open('', 'print', 'width=700,height=1000,left=10,top=10,scrollbars=yes,resizable=yes,directories=no,location=no,menubars=no,status=no');

	with(frm){
		/*arg.value = encodeURIComponent(argVal);*/
		if(type == "agree_1"){			
			file.value = "rpt2-1.jef";	// 개인정보 수집 및 이용 동의서
		}else if(type == "agree_2"){
			file.value = "rpt2-2.jef";	// 연구 참여 동의서3
		}else if(type == "survey"){
			file.value = "rpt3.jef";	// 설문지
		}else if(type == "survey_1"){
			file.value = "rpt3_1.jef";	// 사용성, 효능 설문 조사
		}else if(type == "survey_2"){
			file.value = "rpt4.jef";	// 연구자특성
		}else if(type == "survey_3"){
			file.value = "rpt5.jef";	// 피부자극평가			
		}else if(type == "report"){
		//	file.value = "rpt1.jrf,rpt1-1.jrf,rpt1-2.jrf,rpt1-3.jrf"; // 보고서
			action = "/ubi4/ubihtml.jsp?file=rpt1.jrf,rpt1-1.jrf,rpt1-2.jrf,rpt1-3.jrf&arg="+encodeURIComponent(argVal)+"&"+"exportNm="+encodeURIComponent(exportNm);
		}else if(type == "report2"){
		//	file.value = "rpt1.jrf,rpt1-1.jrf,rpt1-2.jrf,rpt1-3.jrf"; // 별첨1.연구대상자 피부특성
			action = "/ubi4/ubihtml.jsp?file=rpt1-1.jrf&arg="+encodeURIComponent(argVal)+"&"+"exportNm="+encodeURIComponent(exportNm);
		}
		
		if(type != "report"){
			if(type != "report2"){
				action = "/ubi4/ubihtml.jsp?arg="+encodeURIComponent(argVal);
				/*action = "/ubi4/ubihtml.jsp?arg="+argVal;*/
			}
		}
...
function fn_ubi_pop2(frmName, type, argVal, exportNm, tempNo) {
	//alert("frmName= "+frmName);
	//alert("type= "+type);
	//alert("argVal= "+argVal);
	//alert("exportNm= "+exportNm);
	//alert("tempNo= "+tempNo);
	var tempNo1 = tempNo;
	var frm = document.getElementById(frmName);
	var winObj = window.open('', 'print', 'width=700,height=1000,left=10,top=10,scrollbars=yes,resizable=yes,directories=no,location=no,menubars=no,status=no');
	...
	with(frm){
		$.ajax({
			url: '/qxsepmny/cmm/CmmAjaxSearchTmplFile.do'
			, type: "post"
			, data: "type="+tempNo1
			, dataType:"json"
			, success: function(data){
				resultList = "/TMPL/"+data.resultList;
				//alert("resultList= "+resultList);
				file.value = resultList;
		...
		//file.value = fn_search_tmplfile(type);	 
		//alert("file.value= "+file.value);
		
		//file.value = "/TMPL/20210506031800354.jef";
		//action = "/ubi4/ubihtml.jsp?arg="+encodeURIComponent(argVal);
		//target = "print";
		//submit();
...
function fn_ubi_pop3(frmName,type,argVal,fileNm) {
	var frm = document.getElementById(frmName);
	alert("ctms pop3"+fileNm);
	alert("ctms pop3 argVal"+argVal);
	
	var winObj = window.open('', 'print', 'width=700,height=1000,left=10,top=10,scrollbars=yes,resizable=yes,directories=no,location=no,menubars=no,status=no');

	with(frm){
		/*arg.value = encodeURIComponent(argVal);*/
		alert("file type= "+type);
		if(type == "agree_1"){			
			file.value = "rpt2-1.jef";	// 개인정보 수집 및 이용 동의서
		}else if(type == "agree_2"){
			file.value = "rpt2-2.jef";	// 연구 참여 동의서3
			...
			file.value = "rpt3_1.jef";	// 피부특성, 사용성, 효능 설문 조사
		}else if(type == "survey_2"){
			file.value = "rpt4.jef";	// 연구자특성
			alert("file value= "+file.value);
		}else if(type == "survey_3"){
		...
		if(type != "report"){
			if(type != "report2"){
				//action = "/ubi4/ubihtml.jsp?arg="+encodeURIComponent(argVal)+"&file="+fileNm;
				action = "/ubi4/ubihtml.jsp?arg="+encodeURIComponent(argVal);
				/*action = "/ubi4/ubihtml.jsp?arg="+argVal;*/
			}
		}
...
function fn_search_tmplfile(type){
	$.ajax({
		url: '/qxsepmny/cmm/CmmAjaxSearchTmplFile.do'
		, type: "post"
		, data: "type="+type
        ...
        build: function ($trigger) {
            ...
            var options = {
                ...
                callback: function (key, opts, e) {
                    console.log(key);
                    if (key == "1") {
                        $("#listForm").attr("action","/qxsepmny/ech0401/ech0401List.do").submit();
                    }
                    if (key == "2") {
                        $("#listForm").attr("action","/qxsepmny/ech0105/ech0105List.do").submit();
                    }
                    if (key == "3") {
                        $("#listForm").attr("action","/qxsepmny/ech0211/ech0211List.do").submit();
                    }
                    if (key == "4") {
                        $("#listForm").attr("action","/qxsepmny/ech0210/ech0210List.do").submit();
                    }
                    if (key == "5") {
                        $("#listForm").attr("action","/qxsepmny/ech0704/ech0704List.do").submit();
                    }
                    if (key == "6") {
                        $("#listForm").attr("action","/qxsepmny/lgn/admLogout.do").submit();
                    }
...
function fn_file_add(fileKey){
	...
	if(fileCheck_adm2(fileKey)){
		...
		html+= '<a href="#" onclick="fn_uploadFile();" style="color:red;">양식업로드</a>';
		html+= '</div>'; 
...
function fn_filedownload(attachSeq, boardType){
	...
	location.href = "/cmmn/file/downloadFile.do?fileId="+attachSeq+"&type="+boardType;
}
