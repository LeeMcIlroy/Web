	...
	<form:form commandName="sb1000mVO" id="frm" name="frm" method="post">
		...
		<div class="login">
			...
			<fieldset id="idlogin">
				...
				<!--<form:input path="userId" placeholder="아이디" class="required" title="아이디" value="aid" onkeydown="fn_keydown(); return false;"/> -->
				<%-- <form:input path="userId" placeholder="아이디" class="required" title="아이디" value="aid" /> --%>
				<form:input path="userId" placeholder="아이디" class="required" title="아이디" />
				<label for="pwNo" class="hidden">비밀번호</label>
				<%-- <form:password path="pwNo" placeholder="비밀번호" class="required" title="비밀번호" value="aid" /> --%>
				<form:password path="pwNo" placeholder="비밀번호" class="required" title="비밀번호" />
				<p>또는 아래에 핸드폰번호를 입력하세요('-'빼고 입력)<p>
				<p>핸드폰로그인시 일부 기능이 제한됩니다.</p>
				<label for="hpNo" class="hidden">핸드폰번호</label>
				<form:input path="hpNo" placeholder="핸드폰번호" class="required" title="핸드폰번호" value="" />
