...
<script type="text/javascript">
	...
	$(function(){
		...
		var ynAdmin = '<c:out value="${cs2020mVO.isAdminType}"/>';
		var ynDrt = '<c:out value="${cs2020mVO.isRsDrt}"/>';
		var ynStaff = '<c:out value="${cs2020mVO.isRsStaff}"/>';
		...
		//삭제불가능 "N" - 삭제 x
		var ynDel = '<c:out value="${cs2020mVO.isDelCntr}"/>';
		if(ynDel=="N") {
			$("#btnDel").hide();
		}
		
		//var inTamt = '<c:out value="${cs2020mVO.inTamt}"/>';
		//alert(inTamt.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","));
		//$("#inTamt").val(inTamt.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","))
	});

	function fn_list(){
		$("#frm2").attr("action","<c:url value='/qxsepmny/ech0904/ech0904List.do'/>").submit();
	}	
	
	function fn_modify(){
	 	$("#frm").attr("action","<c:url value='/qxsepmny/ech0904/ech0904Modify.do'/>").submit();
	}
	
	function fn_delete(){
		if(confirm('해당 입금정보를 삭제하시겠습니까?')){
			$("#frm").attr("action","<c:url value='/qxsepmny/ech0904/ech0904Delete.do'/>").submit();
		}
	}
...
<div class="wrap">
	...
	<div class="container">
		...
		<div class="contents">
            ...
            <!-- //검색조건유지 설정 -->
            <form:form commandName="cs2020mVO" id="frm" name="frm">
            ...
            <table class="tbl_view">
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                        	<div class="date_sec">
                                <span><c:out value="${cs2020mVO.inDt }"/></span>
                            </div>
                        </td>    
                        <th scope="row" class="bl">입금담당자</th>
                        <td><c:out value="${cs2020mVO.empName }"/></td>
                    </tr>
                    <tr>
                        <th scope="row">계약번호</th>
                        <td><c:out value="${cs2020mVO.ctrtCd }"/></td>
                        <th scope="row" class="bl">고객사</th>
                        <td><c:out value="${cs2020mVO.vendName }"/></td>
                    </tr>
                    <tr>
                        <th scope="row">계약명</th>
                        <td><c:out value="${cs2020mVO.ctrtName }"/></td>
                        <th scope="row" class="bl">계약금액(부가세)</th>
                        <td><c:out value="${cs2020mVO.rsPay }"/>원(<c:out value="${cs2020mVO.rsPayvt }"/>)</td>    
                    </tr>                    
                    <tr>
                    	<th scope="row">입금차수</th>
                        <td><c:out value="${cs2020mVO.inSq }"/></td>
                        <th scope="row" class="bl">입금액</th>
                        <td><c:out value="${cs2020mVO.inAmt }"/>원</td>
                    </tr>                    
                    <tr>
                        <th scope="row">세금계산서일</th>
                        <td colspan="3">
                        	<div class="date_sec">
                                <span><c:out value="${cs2020mVO.reciDt }"/></span>
                            </div>
                        </td>
                    <tr>
                        <th scope="row">비고</th>
                        <td colspan="3" >
                        <textarea><c:out value="${cs2020mVO.remk }"/></textarea>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row">담당지사</th>
                        <td colspan="3" class="p15"><c:out value="${cs2020mVO.branchName }"/></td>
                    </tr>
                    <tr>
                    	<th scope="row">누적입금액</th>
                        <td><c:out value="${cs2020mVO.inTamt }"/>원</td>
                        <th scope="row" class="bl">잔금</th>
                        <td><c:out value="${cs2020mVO.inBamt }"/>원</td>
                    </tr>
                    <tr>
                        <th scope="row">관리번호</th>
                        <td colspan="3"><c:out value="${cs2020mVO.inNo }"/></td>    
                    </tr>
                </tbody>
            </table>
