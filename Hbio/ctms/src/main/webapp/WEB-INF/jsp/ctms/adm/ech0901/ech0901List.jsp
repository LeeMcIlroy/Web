...
<script type="text/javascript"> 
	...
	function fn_view(corpCd, csNo){
		$("#corpCd").val(corpCd);
	 	$("#csNo").val(csNo);
	 	//var str = $("#searchWord").val();
		//$("#searchWord").val(encodeURIComponent(str));
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0901/ech0901View.do'/>").submit();
	}
	
	//등록 화면
	function fn_modify(){
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0901/ech0901Modify.do'/>").submit();
	}
		
	//일괄등록 화면 전환
	function fn_AddAllUpload(){
		if(confirm('(상담)일괄등록 화면으로 전환됩니다. \r\n진행하시겠습니까?')){
			$("#uploadFlag").val("CS");
	...
	function fn_excelDown(){
		if(confirm('출력하는 데이터건수에 따라 시간이 지체될 수 있습니다. \r\n진행하시겠습니까?')){
			$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0901/ech0901Excel.do'/>").submit();
		}
	}
	...
	function fn_del(){
		...
		if($("input[name=keySeq]:checked").length == 0){
			alert('삭제할 상담 정보를 선택해 주세요.');
			return;
		}
		if(confirm('선택하신 상담 정보를 삭제합니다. 삭제한 정보는 복구할 수 없습니다. \r\n삭제하시겠습니까?')){
			var corpCd = $("#corpCd").val();
			var step1 = "Y";
			var step2 = "N";
			$.ajax({
				url: "<c:url value='${pageContext.request.contextPath }/qxsepmny/ech0901/ech0901AjaxCsDel.do'/>"
				, type: "post"
	...
	function fn_delOne(paramKeyNo){
		...
		if(confirm('상담 정보를 삭제합니다. 삭제한 정보는 복구할 수 없습니다. \r\n삭제하시겠습니까?')){
			var corpCd = $("#corpCd").val();
			var step1 = "Y";
			var step2 = "N";
		...
		<div class="contents">
			...
			<!-- //타이틀 -->
						...
						<li>
	                        ...
	                        <span>
	                        	<select name="searchCondition4" id="searchCondition4" onchange="fn_resetTerm(); return false;">
									...
									<option value="1" <c:if test="${searchVO.searchCondition4  eq '1' }">selected="selected"</c:if> >상담일</option>
								</select>
	                        </span>
							<span>
	                    ...
	                    <li>
							...
							<span>
								<select name="searchCondition3" id="searchCondition3" onchange="fn_resetWord(); return false;">
									...
									<option value="2" <c:if test="${searchVO.searchCondition3  eq '2' }">selected="selected"</c:if> >상담내용</option>
									<option value="3" <c:if test="${searchVO.searchCondition3  eq '3' }">selected="selected"</c:if> >고객사명</option>
									<option value="4" <c:if test="${searchVO.searchCondition3  eq '4' }">selected="selected"</c:if> >상담자</option>
								</select>
							</span>
	                        <span class="type02">
	                            <input type="text" name="searchWord" id="searchWord" value="${searchVO.searchWord }" class="input-data" placeholder="상담내용,고객사명,상담자를 입력하세요" style="width: 248px;"/>
	                        </span>
						</li>
					</ul>
					<!-- 조회버튼 -->
				...
				<div class="tbl_top_info">
					...
					<!-- 버튼 -->
					<div>
						<a href="#" class="btn_sub" onclick="fn_regVend(); return false;" id="btnDel">고객사 간편등록</a>
						...
						<!-- <a href="#" onclick="fn_setMyMenu(); return false;" class="btn_sub">마이메뉴등록</a> -->
					</div>
				</div>
				<!-- //테이블 상단 정보 -->
				<!-- 목록 -->
				<table class="tbl_list tr_link">
					...
					<tbody>
						<c:forEach items="${resultList }" var="result" varStatus="status">
							<tr id="contextMenu">
								<td><input type="checkbox" name="keySeq" value="<c:out value='${result.csNo }'/>"/></td>
								<td><c:out value="${paginationInfo.totalRecordCount+1 - ((searchVO.pageIndex-1) * paginationInfo.recordCountPerPage + status.count)}"/></td>
								<td><c:out value="${result.csDt }"/></td>
								<td class="keyv" onclick="fn_view('<c:out value='${result.corpCd}'/>','<c:out value='${result.csNo}'/>'); return false;" style="text-decoration: underline;"><span style="display:none"><c:out value="${result.csNo }"/></span><c:out value="${result.vendName }"/></td>
		         	            <td class="keyv" onclick="fn_view('<c:out value='${result.corpCd}'/>','<c:out value='${result.csNo}'/>'); return false;" style="text-decoration: underline;"><span style="display:none"><c:out value="${result.csNo }"/></span><c:out value="${result.csCont }"/></td>
		         	            <td><c:out value="${result.rcsName }"/></td>
		                        <td><c:out value="${result.rcsTel }"/></td>
		                        <td><c:out value="${result.rcsEmail }"/></td>
		                        <td><c:out value="${result.csClsNm }"/></td>
						...
						<c:if test="${resultList.size() == 0 }">
							<tr>
								<td class="nodata" colspan="11">상담정보가 없습니다.</td>
							</tr>
						</c:if>
					</tbody>
				</table>
				...
				<div class="list_btm_btn">
					...
					<a href="#" onclick="fn_AddAllUpload(); return false;">상담일괄등록</a>
				</div>
				<!-- //하단버튼 -->
			</div>
			<!-- //목록 하단 -->
