		...
		<div class="contents">
            ...
            <div class="tab_area tab04">
                <ul>
                	...
                	<li><a href="#" onclick="fn_view6(); return false;" class="on">결과보고서(안전성)</a></li>
                </ul>
            </div>
            <!-- //탭버튼 -->
			...
			<table class="tbl_list">
				...
				<c:forEach var="result" items="${resultList}" varStatus="status">
					<tr>
						<td><input type="checkbox" name="mtlSeq" value="<c:out value='${result.rsNo }'/>"/></td>
						<td><c:out value="${result.rsNo7 }"/></td>
						...
						<td><a href="#" onclick="fn_ubi_pop('detailForm', 'report', 'corpCd#<c:out value='${result.corpCd }'/>#rsNo#<c:out value='${result.rsNo }'/>#mrsNo#<c:out value='${result.mrsNo }'/>','<c:out value='${result.rsCd }'/>_결과보고서_<c:out value='${result.vendName }'/>'); return false;" class="btn_sub btnLockNonDisp" >결과보고서 추출</a>
						</td>
					</tr>
				</c:forEach>
				<c:if test="${resultList.size() == 0 }">
					<tr>
						<td class="nodata" colspan="6">등록된 연구차수 정보가 없습니다.</td>
					</tr>
				</c:if>
				</tbody>
			</table>
