...
<script type="text/javascript"> 
                ...
                var options = {
                    ...
                    callback: function (key, opts, e) {
                        ...
                        if (key == "view") {
                            ...
                            var paramKeyNo = str.substr(0,8);
                            $("#inNo").val(paramKeyNo);
                            /* var actionUrl = "<c:url value='/qxsepmny/ech0904/ech0904View.do'/>"+"?paramKeyNo="+paramKeyNo; */
                            var actionUrl = "<c:url value='/qxsepmny/ech0904/ech0904View.do'/>";
                        ...
                        if (key == "edit") {
                            ...
                            var paramKeyNo = str.substr(0,8);
                            $("#inNo").val(paramKeyNo);
                            /* var actionUrl = "<c:url value='/qxsepmny/ech0904/ech0904Modify.do'/>"+"?paramKeyNo="+paramKeyNo; */
	...
	function fn_view(corpCd, inNo){
		$("#corpCd").val(corpCd);
	 	$("#inNo").val(inNo);
	 	//var str = $("#searchWord").val();
		//$("#searchWord").val(encodeURIComponent(str));
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0904/ech0904View.do'/>").submit();
	}
	
	//SMS 테스트
	function fn_modify1(){
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0904/ech0904Sms.do'/>").submit();
	}

	//등록 화면
	function fn_modify(){
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0904/ech0904Modify.do'/>").submit();
	}
	
	/* //입금일괄등록 화면
	function fn_InUpload(){
		if(confirm('일괄등록 화면으로 전환됩니다. \r\n진행하시겠습니까?')){
	 		$("#listForm").attr("action","<c:url value='/qxsepmny/ech0904/ech0904InUpload.do'/>").submit();
		}	
	} */
	
	//일괄등록 화면 전환
	function fn_AddAllUpload(){
		if(confirm('(입금)일괄등록 화면으로 전환됩니다. \r\n진행하시겠습니까?')){
			$("#uploadFlag").val("IN");
	...
	function fn_excelDown(){
		if(confirm('출력하는 데이터건수에 따라 시간이 지체될 수 있습니다. \r\n진행하시겠습니까?')){
			$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0904/ech0904Excel.do'/>").submit();
		}
	}
	...
	function fn_del(){
		...
		if($("input[name=keySeq]:checked").length == 0){
			alert('삭제할 입금 정보를 선택해 주세요.');
			return;
		}
		if(confirm('선택하신 입금 정보를 삭제합니다. 삭제한 정보는 복구할 수 없습니다. \r\n삭제하시겠습니까?')){
			var corpCd = $("#corpCd").val();
			var step1 = "Y";
			var step2 = "N";
			$.ajax({
				url: "<c:url value='${pageContext.request.contextPath }/qxsepmny/ech0904/ech0904AjaxInDel.do'/>"
				, type: "post"
	...
	function fn_delOne(paramKeyNo){
		...
		if(confirm('입금 정보를 삭제합니다. 삭제한 정보는 복구할 수 없습니다. \r\n삭제하시겠습니까?')){
			var corpCd = $("#corpCd").val();
			var step1 = "Y";
			var step2 = "N";
		...
		<div class="contents">
			...
			<form:form commandName="searchVO" id="listForm" name="listForm">
						...
						<li>
	                        ...
	                        <span>
	                        	<select name="searchCondition4" id="searchCondition4" onchange="fn_resetTerm(); return false;">
									...
									<option value="1" <c:if test="${searchVO.searchCondition4  eq '1' }">selected="selected"</c:if> >입금일</option>
	                    ...
	                    <li>
							...
							<span>
								<select name="searchCondition3" id="searchCondition3" onchange="fn_resetWord(); return false;">
									...
									<option value="4" <c:if test="${searchVO.searchCondition3  eq '4' }">selected="selected"</c:if> >계약번호</option>
								</select>
							</span>
	                        <span class="type02">
				...
				<table class="tbl_list tr_link">
					...
					<tbody>
						<c:forEach items="${resultList }" var="result" varStatus="status">
							<tr id="contextMenu">					
								<td><input type="checkbox" name="keySeq" value="<c:out value='${result.inNo }'/>"/></td>
								<td><c:out value="${paginationInfo.totalRecordCount+1 - ((searchVO.pageIndex-1) * paginationInfo.recordCountPerPage + status.count)}"/></td>
								<td class="keyv" onclick="fn_view('<c:out value='${result.corpCd}'/>','<c:out value='${result.inNo}'/>'); return false;" style="text-decoration: underline;"><span style="display:none;"><c:out value='${result.inNo }'/></span><c:out value="${result.ctrtCd }"/></td>
								<td><c:out value="${result.ctrtClsNm }"/></td>
								<td><c:out value="${result.vendName }"/></td>
								<td class="keyv" onclick="fn_view('<c:out value='${result.corpCd}'/>','<c:out value='${result.inNo}'/>'); return false;" style="text-decoration: underline;"><span style="display:none;"><c:out value='${result.inNo }'/></span><c:out value="${result.ctrtName }"/></td>
								<td><c:out value="${result.inDt }"/></td>
								<td><c:out value="${result.inSq }"/></td>
								<td class="txt-r"><fmt:formatNumber value="${result.inAmt }" pattern="#,###"/></td>
								<td class="txt-r"><fmt:formatNumber value="${result.inTamt }" pattern="#,###"/></td>
								<td class="txt-r"><fmt:formatNumber value="${result.inBamt }" pattern="#,###"/></td>
								<%-- <td><c:out value="${result.inAmt }"/></td>
								<td><c:out value="${result.inTamt }"/></td>
								<td><c:out value="${result.inBamt }"/></td> --%>
						...
						<c:if test="${resultList.size() == 0 }">
							<tr>
								<td class="nodata" colspan="12">입금정보가 없습니다.</td>
							</tr>
						</c:if>
					</tbody>
				</table>
				...
				<div class="list_btm_btn">
					...
					<a href="#" onclick="fn_AddAllUpload(); return false;">입금일괄등록</a>
				</div>
				<!-- //하단버튼 -->
			</div>
			<!-- //목록 하단 -->
