...
<script type="text/javascript">
	function fn_list(pageNo){
		...
		$("#frm").attr("action", "<c:url value='/qxsepmny/ech0201/ech0201List.do'/>").submit();
	}

	function fn_view(rsNo){
		$("#rsNo").val(rsNo);
		$("#frm").attr("action", "<c:out value='${pageContext.request.contextPath}/qxsepmny/ech0201/ech0201Modify.do'/>").submit();
	}
				...
				<table class="tbl_list tr_link">
					...
					<tbody>
						<c:choose>
							...
							<c:otherwise>
								<tr>
									<td class="nodata" colspan="8">유효성설문관리 정보가 없습니다.</td>
								</tr>
							</c:otherwise>
						</c:choose>
					</tbody>
