...
<script type="text/javascript">	
	...
	//SMS메시지관리 상세보기 화면
	function fn_view(corpCd, smstNo){
	 	$("#corpCd").val(corpCd);
	 	$("#smstNo").val(smstNo);	 	
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0604/ech0604View.do'/>").submit();
	}
	
	//SMS메시지관리 목록으로
	function fn_list(pageNo){
		$("#pageIndex").val(pageNo);
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0604/ech0604List.do'/>").submit();
	}
	//SMS메시지관리 등록화면
	function fn_regist(){
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0604/ech0604Modify.do'/>").submit();
	}
	
	//SMS메시지관리 엑셀다운로드
	function fn_excelDown(){
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0604/ech0604Excel.do'/>").submit();
	}
</script>
<body>
<form:form commandName="searchVO" id="listForm" name="listForm">
<input type="hidden" id="corpCd" name="corpCd"/>
<input type="hidden" id="smstNo" name="smstNo"/>
<!-- wrap -->
<div class="wrap">
	...
	<div class="container">
		...
		<div class="contents">
			...
			<jsp:include page="/WEB-INF/jsp/ctms/adm/inc/incPageTitle.jsp">
	            ...
	            <jsp:param name="dept2" value="SMS메시지"/>
           	</jsp:include>
			<!-- //타이틀 -->
			<!-- 검색영역 -->
			<div class="srch_area">
			...
			<table class="tbl_list tr_link">
				...
				<c:forEach var="result" items="${resultList}" varStatus="status">			
					<tr onclick="fn_view('<c:out value="${result.corpCd }"/>', '<c:out value="${result.smstNo }"/>'); return false;">
				...
				<c:if test="${resultList.size() == 0 }">
					<tr>
						<td class="nodata" colspan="4">SMS메시지 정보가 없습니다.</td>
					</tr>
				</c:if>
				</tbody>
			</table>
