											...
											<div>
												...
												<c:if test="${result.dtState eq 'NOW' }">
													...
													<c:if test="${result.answState eq 'FALSE' }">
														<a href="#" onclick="fn_ubi_pop('frm', 'survey', 'corpCd#<c:out value='${result.corpCd }'/>#rsNo#<c:out value='${result.rsNo }'/>#tempNo#<c:out value='${result.tempNo }'/>#rsiNo#<c:out value='${rsiNo }'/>#rsjNo#<c:out value='${rsjNo }'/>#type#survey'); return false;" class="type01">참여하기</a>
													</c:if>
												</c:if>
