...
<script type="text/javascript">
	...
	function fn_view(corpCd, rsNo){		
	 	$("#corpCd").val(corpCd);
	 	$("#rsNo").val(rsNo);
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0212/ech0212View.do'/>").submit();
	}
	
	//제품사용관리 목록으로
	function fn_list(pageNo){
		$("#pageIndex").val(pageNo);
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0212/ech0212List.do'/>").submit();
	}
		...
		<div class="contents">
			...
			<table class="tbl_list tr_link">
				...
				<c:if test="${resultList.size() == 0 }">
					<tr>
						<td class="nodata" colspan="8">CRF작성 정보가 없습니다.</td>
					</tr>
				</c:if>
				</tbody>
			</table>
