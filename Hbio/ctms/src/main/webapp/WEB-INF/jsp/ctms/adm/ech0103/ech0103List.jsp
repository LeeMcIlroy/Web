...
<script type="text/javascript">
	...
	function fn_pop_smsmt(){
		...
		window.open("<c:out value='${pageContext.request.contextPath }/qxsepmny/ech0103/ech0103SmsMtPop.do'/>?corpCd="+corpCd+"&"+$("input[name=rsjSeq]:checked").serialize()
	...
	function fn_pop_email(){
		...
		if($("input[name=rsjSeq]:checked").length > 5){
			alert('메일발송대사자는 5명이하로 선택해주세요.');
			return;
		}
		var corpCd = $("#corpCd").val();
		window.open("<c:out value='${pageContext.request.contextPath }/qxsepmny/ech0103/ech0103EmailAllPop.do'/>?corpCd="+corpCd+"&"+$("input[name=rsjSeq]:checked").serialize()
					, '이메일발송', 'width=840, height=500, menubar=no, status=no, toolbar=no, scrollbars=1');
	}
	...
	//일괄 예약관리 팝업
	function fn_mtmgallpop(){
		if($("input[name=rsjSeq]:checked").length == 0){
			alert('연구대상자를 선택해주세요.');
			return;
		}
		var rsjNo = $("input[name=rsjSeq]:checked").serialize();
		window.open("<c:out value='${pageContext.request.contextPath }/qxsepmny/ech0103/ech1003MtmgAllPop.do'/>?corpCd="+corpCd+"&"+$("input[name=rsjSeq]:checked").serialize()
				, '예약관리', 'width=1000, height=500, menubar=no, status=no, toolbar=no, scrollbars=1');
	}
	
	//SMS관리 상세보기 화면
	function fn_view(corpCd, rsNo, rvNo){		
	 	$("#corpCd").val(corpCd);
	 	$("#rsNo").val(rsNo);
	 	$("#rvNo").val(rvNo);
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0103/ech0103View.do'/>").submit();
	}
	
	//SMS관리 목록으로
	function fn_list(pageNo){
		$("#pageIndex").val(pageNo);
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0103/ech0103List.do'/>").submit();
	}
	
	//SMS관리 등록화면
	function fn_regist(){
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0103/ech0103Modify.do'/>").submit();
	}
	
	//SMS관리 엑셀다운로드
	function fn_excelDown(){
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0103/ech0103Excel.do'/>").submit();
	}
	...
	//예약수정
	function fn_resrModiPop(corpCd, resrNo){
		window.open("<c:out value='${pageContext.request.contextPath }/qxsepmny/ech0103/ech1003MtmgPop.do'/>?corpCd="+corpCd+"&resrNo="+resrNo
				, '예약관리', 'width=500, height=500, menubar=no, status=no, toolbar=no, scrollbars=1');
	}
...
<input type="hidden" id="resrNo" name="resrNo"/>
<!-- wrap -->
<div class="wrap">
	...
	<div class="container">
		...
		<div class="contents">
                    ...
                    <li>
						...
						<span>
							<select name="searchCondition3" id="searchCondition3" onchange="fn_resetWord(); return false;">
								...
								<option value="2" <c:if test="${searchVO.searchCondition3  eq '2' }">selected="selected"</c:if> >대상자명</option>
								<option value="2" <c:if test="${searchVO.searchCondition3  eq '3' }">selected="selected"</c:if> >연구명칭</option>
								<option value="3" <c:if test="${searchVO.searchCondition3  eq '4' }">selected="selected"</c:if> >고객사명</option>
								<option value="3" <c:if test="${searchVO.searchCondition3  eq '5' }">selected="selected"</c:if> >연구코드</option>
							</select>
						</span>
                        <span class="type02">
                            <input type="text" name="searchWord" class="input-data" placeholder="대상자명,연구명칭,고객사명,연구코드를 입력하세요" style="width: 248px;"/>
                        </span>
					</li>				
				</ul>
				<!-- 조회버튼 -->
			...
			<div class="tbl_top_info">
				...
				<!-- 버튼 -->
				<div>
					<a href="#" onclick="fn_pop_smsmt();return false;" class="btn_sub">예약SMS 발송</a>
					...
					<!-- <a href="#" onclick="fn_pop1('<c:out value="${result.corpCd }"/>','<c:out value="${result.rsNo }"/>','<c:out value="${result.rsjNo }"/>'); return false;" class="btn_sub">SMS관리</a> -->
					<a href="#" onclick="fn_mtmgallpop('<c:out value="${result.corpCd }"/>','<c:out value="${result.rsNo }"/>','<c:out value="${result.rsjNo }"/>'); return false;" class="btn_sub">일괄예약관리</a>
...
<!-- 					<tr onclick="fn_view('<c:out value='${result.corpCd}'/>','<c:out value='${result.rsjNo}'/>','<c:out value='${result.rsjNo}'/>'); return false;"> -->
					<tr>
						...
						<td><c:out value="${result.tmtCnt }"/></td>
						<%-- <td><c:out value="${result.rsCnt }"/></td> --%>
						<td id="answTd_0">
							...
							<c:forEach items="${mtList[chkNo] }" var="resutlMt" varStatus="status">
								<div class="que_item ques_div_0">
									<a href="#" onclick="fn_resrModiPop('<c:out value="${resutlMt.corpCd }"/>','<c:out value="${resutlMt.resrNo }"/>'); return false;" style="text-decoration: underline;" ><c:out value="${resutlMt.resrDt }"/>-<c:out value="${resutlMt.resrHr }"/>:<c:out value="${resutlMt.resrMm }"/>(
										<%-- <c:choose>
	                          				<c:when test="${resutlMt.mtStNm eq '예약취소' }"><span style="color:red;">예약취소</span></c:when>
	                          				<c:otherwise><c:out value="${resutlMt.mtStNm }"/></c:otherwise>
		                          		</c:choose> --%>										
		                          		<c:out value="${resutlMt.mtStNm }"/>
									)</a>						
								</div>
							</c:forEach>
						</td>
				...
				<c:if test="${resultList.size() == 0 }">
					<tr>
						<td class="nodata" colspan="13">예약 정보가 없습니다.</td>
					</tr>
				</c:if>				
				</tbody>
			</table>
