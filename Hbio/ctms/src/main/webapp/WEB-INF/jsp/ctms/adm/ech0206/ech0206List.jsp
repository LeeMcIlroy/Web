...
<script type="text/javascript">
	...
	function fn_view(corpCd, rsNo){		
	 	$("#corpCd").val(corpCd);
	 	$("#rsNo").val(rsNo);
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0206/ech0206View.do'/>").submit();
	}
	
	//연구동의서관리 목록으로
	function fn_list(pageNo){
		$("#pageIndex").val(pageNo);
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0206/ech0206List.do'/>").submit();
	}
	
	//연구동의서관리 등록화면
	function fn_regist(){
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0206/ech0206Modify.do'/>").submit();
	}
	
	//연구동의서관리 엑셀다운로드
	function fn_excelDown(){
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0206/ech0206Excel.do'/>").submit();
	}
			...
			<table class="tbl_list tr_link">
				...
				<c:forEach var="result" items="${resultList}" varStatus="status">
					<tr onclick="fn_view('<c:out value='${result.corpCd}'/>','<c:out value='${result.rsNo}'/>'); return false;">					
						...
						<td><c:out value="${result.pricfCount}/${result.rsScnt}"/></td>
						<td><c:out value="${result.rsicfCount}/${result.rsScnt}"/></td>
					</tr>
				</c:forEach>
				<c:if test="${resultList.size() == 0 }">
							<tr>
								<td class="nodata" colspan="9">연구과제 정보가 없습니다.</td>
							</tr>
					</c:if>
				</tbody>
			</table>
