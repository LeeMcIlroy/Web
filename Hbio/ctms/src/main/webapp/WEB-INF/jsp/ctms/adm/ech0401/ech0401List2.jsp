...
<script type="text/javascript">
	...
	function fn_view(corpCd, rsNo){		
	 	$("#corpCd").val(corpCd);
	 	$("#rsNo").val(rsNo);
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0401/ech0401ViewIcf.do'/>").submit();
	}
	...
	//연구동의서관리 등록화면
	function fn_regist(){
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0401/ech0401Modify.do'/>").submit();
	}
	
	//연구동의서관리 엑셀다운로드
	function fn_excelDown(){
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0401/ech0401Excel.do'/>").submit();
	}
			...
			<table class="tbl_list tr_link">
				...
				<thead>
					<tr>
						...
						<th scope="col">동의서(개인)</th>
						<th scope="col">동의서(연구)</th>
						<th scope="col">CRF(설문)</th>
				...
				<c:forEach var="result" items="${resultList}" varStatus="status">
					<tr onclick="fn_view('<c:out value='${result.corpCd}'/>','<c:out value='${result.rsNo}'/>'); return false;">					
						...
						<td><c:out value="${result.rsScnt}"/></td> <%-- 피험자수 --%>  
						<td><c:out value="${result.pricfCount}/${result.rsScnt}"/></td> <%-- 개인정보동의 V--%>
						<td><c:out value="${result.rsicfCount}/${result.rsScnt}"/></td> <%-- 연구참여동의 V--%>
						<td><c:out value="${result.svyCount}/${result.rsScnt}"/></td> <%-- CRF(설문) --%>
						<td><c:out value="${result.svy2Count}/${result.rsScnt}"/></td> <%-- 피부특성 --%>
						<td><c:out value="${result.skinCount}/${result.rsScnt}"/></td> <%-- 피부자극 V--%>
						<td><c:out value="${result.useCount}/${result.rsScnt}"/></td> <%-- 제품사용 V--%>
					</tr>
				</c:forEach>
				<c:if test="${resultList.size() == 0 }">
					<tr>
						<td class="nodata" colspan="13">연구과제 정보가 없습니다.</td>
					</tr>
				</c:if>
				</tbody>
			</table>
