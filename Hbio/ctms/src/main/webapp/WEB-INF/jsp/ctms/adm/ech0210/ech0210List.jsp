...
<script type="text/javascript"> 
            ...
            build: function ($trigger) {
                ...
                var options = {
                    ...
                    callback: function (key, opts, e) {
                        console.log(key);
                        if (key == "설정관리") {
                            $("#listForm").attr("action","<c:url value='/qxsepmny/ech0901/ech0211List.do'/>").submit();
                        }
                        if (key == "view") {
                        	var str = $trigger.text();
                            var paramKeyNo = str.substr(0,8);
                            $("#tempNo").val(paramKeyNo);
                            /* var actionUrl = "<c:url value='/qxsepmny/ech0210/ech0210View.do'/>"+"?paramKeyNo="+paramKeyNo; */
                            var actionUrl = "<c:url value='/qxsepmny/ech0210/ech0210View.do'/>";
                        ...
                        if (key == "edit") {
                            ...
                            var paramKeyNo = str.substr(0,8);
                            $("#tempNo").val(paramKeyNo);
                            /* var actionUrl = "<c:url value='/qxsepmny/ech0210/ech0210Modify.do'/>"+"?paramKeyNo="+paramKeyNo; */
                            var actionUrl = "<c:url value='/qxsepmny/ech0210/ech0210Modify.do'/>";
                            $("#listForm").attr("action",actionUrl).submit();
                        }
                        if (key == "del") {
                        	var str = $trigger.text();
	...
	function fn_view(corpCd, tempNo){
		$("#corpCd").val(corpCd);
	 	$("#tempNo").val(tempNo);
	 	//var str = $("#searchWord").val();
		//$("#searchWord").val(encodeURIComponent(str));
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0210/ech0210View.do'/>").submit();
	}
	
	//SMS 테스트 - 기능없음
	function fn_modify1(){
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0210/ech0210Sms.do'/>").submit();
	}

	//등록 화면
	function fn_modify(){
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0210/ech0210Modify.do'/>").submit();
	}
	
	//CRF템플릿일괄등록 화면 - 기능없음
	function fn_RsUpload(){
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0210/ech0210RsUpload.do'/>").submit();
	}
	
	//CRF템플릿관리 목록으로
	function fn_list(pageNo){
		$("#pageIndex").val(pageNo);
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0210/ech0210List.do'/>").submit();
	}
	
	//CRF템플릿 목록 엑셀다운로드
	function fn_excelDown(){
		if(confirm('출력하는 데이터건수에 따라 시간이 지체될 수 있습니다. \r\n진행하시겠습니까?')){
			$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0210/ech0210Excel.do'/>").submit();
		}
	}
	...
	function fn_delOne(paramKeyNo){
		...
		if(confirm('CRF템플릿 정보를 삭제합니다. 삭제한 정보는 복구할 수 없습니다. 사용된 CRF템플릿정보는 삭제할 수 없습니다. \r\n삭제하시겠습니까?')){
			...
			var keySeq = [ paramKeyNo ];
			$.ajax({
				url: "<c:url value='${pageContext.request.contextPath }/qxsepmny/ech0210/ech0210AjaxOpDel.do'/>"
				, type: "post"
	...
	function fn_ZipDownload(){
		...
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0210/ech0210ZipDownloadCrfTemp.do'/>").submit();
		...
		<div class="contents">
			...
			<form:form commandName="searchVO" id="listForm" name="listForm" method="post">			
				...
				<div class="srch_area" id="contextMenu2">
					<ul>
	                    ...
	                    <li>
							<p>템플릿분류</p>
							<span>
								<!-- CRF템플릿분류 목록(공통코드) CM4000M - CT3020   -->
						...
						<li>
	                        ...
	                        <span>
	                        	<select name="searchCondition4" id="searchCondition4" onchange="fn_resetTerm(); return false;">
									...
									<option value="1" <c:if test="${searchVO.searchCondition4  eq '1' }">selected="selected"</c:if> >등록일</option>
									<option value="2" <c:if test="${searchVO.searchCondition4  eq '2' }">selected="selected"</c:if> >사용시작일</option>
									<option value="3" <c:if test="${searchVO.searchCondition4  eq '3' }">selected="selected"</c:if> >사용종료일</option>
								</select>
	                        </span>
							<span>
	                    ...
	                    <li>
							...
							<span>
								<select name="searchCondition3" id="searchCondition3" onchange="fn_resetWord(); return false;">
									...
									<option value="2" <c:if test="${searchVO.searchCondition3  eq '2' }">selected="selected"</c:if> >CRF템플릿명</option>
									<option value="3" <c:if test="${searchVO.searchCondition3  eq '3' }">selected="selected"</c:if> >CRF템플릿번호</option>
								</select>
							</span>
	                        <span class="type02">
	                            <input type="text" name="searchWord" id="searchWord" value="${searchVO.searchWord }" class="input-data" placeholder="CRF템플릿명,템플릿번호을 입력하세요" style="width: 248px;"/>
	                        </span>
						</li>
					</ul>
					<!-- 조회버튼 -->
				...
				<div class="tbl_top_info">
					...
					<!-- 버튼 -->
					<div>
						<a href="#" onclick="fn_ZipDownload(); return false;" class="btn_sub">CRF템플릿일괄다운로드(zip)</a>
				...
				<table class="tbl_list tr_link">
					...
					<thead>
						<tr>
							...
							<th scope="col">번호</th>
							<th scope="col">CRF템플릿번호</th>
							<th scope="col">CRF템플릿명</th>
							<th scope="col">분류</th>
					...
					<tbody>
						<c:forEach items="${resultList }" var="result" varStatus="status">
							<tr id="contextMenu">					
								<td><input type="checkbox" name="keySeq" value="<c:out value='${result.tempNo }'/>"/></td>
								<td><c:out value="${paginationInfo.totalRecordCount+1 - ((searchVO.pageIndex-1) * paginationInfo.recordCountPerPage + status.count)}"/></td>
								<td class="keyv" onclick="fn_view('<c:out value='${result.corpCd}'/>','<c:out value='${result.tempNo}'/>'); return false;" style="text-decoration: underline;"><span style="display:none"><c:out value="${result.tempNo }"/></span><c:out value="${result.tempCd }"/></td>
								<td class="keyv" onclick="fn_view('<c:out value='${result.corpCd}'/>','<c:out value='${result.tempNo}'/>'); return false;" style="text-decoration: underline;"><span style="display:none"><c:out value="${result.tempNo }"/></span><c:out value="${result.tempNm }"/></td>
								<td><c:out value="${result.tempTypeNm }"/></td>
								<%-- <td><c:out value="${result.tempNo }"/></td> --%>
								<td><c:out value="${result.regDt }"/></td>
								<td><c:out value="${result.stDate }"/></td>
								<td><c:out value="${result.edDate }"/></td>
								<td><c:out value="${result.empName}"/></td>
								<td><c:out value="${result.branchName}"/></td>
								<td><c:out value="${result.useYn}"/></td>
								<td><c:out value="${result.upageCnt}"/></td>
								<%-- <td><a href="#" onclick="fn_oppop('u','<c:out value="${result.corpCd }"/>','<c:out value="${result.tempNo }"/>'); return false;" class="btn_sub btnModiMtl" >보기</a></td> --%>
								<td>
									<c:choose>
		                          		<c:when test="${result.svyYn eq 'Y' and result.tempType ne '4000'}">
											<a href="#" onclick="fn_ubi_pop('listForm', 'survey_1', 'corpCd#<c:out value='${result.corpCd }'/>#rsNo#<c:out value='${result.rsNo }'/>#tempNo#<c:out value='${result.tempNo }'/>#rsiNo#<c:out value='${result.rsiNo }'/>#rsjNo#<c:out value='${result.rsjNo }'/>#usrName#<c:out value="${result.rsiNo }"/>#type#survey_1'); return false;" class="btn_sub">설문</a>
										</c:when>
										<c:when test="${result.svyYn eq 'Y' and result.tempType eq '4000' }">
											<a href="#" onclick="fn_ubi_pop('listForm', 'survey_2', 'corpCd#<c:out value='${result.corpCd }'/>#rsNo#<c:out value='${result.rsNo }'/>#tempNo#<c:out value='${result.tempNo }'/>#rsiNo#<c:out value='${result.rsiNo }'/>#rsjNo#<c:out value='${result.rsjNo }'/>#usrName#<c:out value="${result.rsiNo }"/>#type#survey_2'); return false;" class="btn_sub">특성</a>
										</c:when>
										<c:when test="${result.svyYn eq 'N' }">
											<!-- 템플릿번호로 파일 찾기 -->
											<a href="#" onclick="fn_ubi_pop2('listForm', <c:out value='${result.tempNo }'/>, 'corpCd#<c:out value='${result.corpCd }'/>#tempNo#<c:out value='${result.tempNo }'/>#type#crf#title#<c:out value='${result.tempNo }'/>', '', <c:out value='${result.tempNo }'/>); return false;" class="btn_sub">보기</a>
										</c:when>
									</c:choose>									
								</td>	
							</tr>
						...
						<c:if test="${resultList.size() == 0 }">
							<tr>
								<td class="nodata" colspan="13">CRF템플릿정보가 없습니다.</td>								
							</tr>
						</c:if>
					</tbody>
				</table>
				...
				<div class="list_btm_btn">
					...
					<!-- <a href="#" onclick="fn_RsUpload(); return false;">CRF템플릿일괄등록</a> -->
				</div>
				<!-- //하단버튼 -->
			</div>
			<!-- //목록 하단 -->
...
<!-- //wrap -->
<%-- <td><a href="#" onclick="fn_ubi_pop2('detailForm', <c:out value='${result.tempNo }'/>, 'corpCd#<c:out value='${result.corpCd }'/>#rsNo#<c:out value='${result.rsNo }'/>#tempNo#<c:out value='${result.tempNo }'/>#rsiNo#<c:out value='${result.rsiNo }'/>#rsjNo#<c:out value='${result.rsjNo }'/>#usrName#<c:out value="${result.rsiNo }"/>#type#crf#title#<c:out value='${result.title }'/>', '', <c:out value='${result.tempNo }'/>); return false;" class="btn_sub">보기</a></td> --%>
</body>
</html>
