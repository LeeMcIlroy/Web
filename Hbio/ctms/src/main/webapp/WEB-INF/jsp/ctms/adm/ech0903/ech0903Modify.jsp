...
<script type="text/javascript">
	...
	$(function(){
		...
		// 회신상태 Y 회신확인, N 미확인
		var ynrtYn = '<c:out value="${cs2000mVO.rtYn}"/>';
	...
	function fn_search3(){
		$.ajax({
			url: "<c:url value='/qxsepmny/ech1005/ech1005AjaxSearchOp.do'/>"
			, type: "post"
			...
			, success: function(data){
				...
				for(var i=0; i<resultList.length; i++){
					...
					html += '	<td>'+resultList[i].vendName+'</td>';
					html += '	<td>'+resultList[i].opCd+'</td>';
					html += '	<td>'+resultList[i].opDt+'</td>';
					html += '	<td>'+resultList[i].opName+'</td>';
	...
	function fn_select3(i){
		...
		//팝업에서 선택한 값을 설정한다.
		$("#opNo").val(result.opNo);
		$("#opCd").val(result.opCd);
		$("#ctrtName").val(result.opName);
		var rsPay = result.rsPay;
		var rsPay2 = rsPay.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
		$("#rsPay").val(rsPay2);
		
		var rsPayvt = result.rsPayvt;
		var rsPayvt2 = rsPayvt.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
		$("#rsPayvt").val(rsPayvt2);
		
		var rsTpay = result.rsTpay;
		var rsTpay2 = rsTpay.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
		$("#rsTpay").val(rsTpay2);
		
		$("#vendNo").val(result.vendNo);
		$("#vendName").val(result.vendName);
		$("#ctrtCls").val(result.opCls);
		
		//업무담당자,연락처,이메일을 설정한다.
		$("#vmngName").val(result.vmngName);
		$("#vmnghpNo").val(result.vmnghpNo);
		$("#vmngEmail").val(result.vmngEmail);
	...
	function fn_list(){
		location.href = "<c:out value='${pageContext.request.contextPath }/qxsepmny/ech0903/ech0903List.do'/>";
	}	
	
	//function fn_update(){
		//if(fn_validate("detailForm")){
		 	//$("#detailForm").attr("action","<c:url value='/qxsepmny/ech0903/ech0903Update.do'/>").submit();
		//}
	//}
	...
	function fn_update(){
		...
		$("#detailForm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath}/qxsepmny/ech0903/ech0903Update.do'/>").submit();
	}
...
<div class="wrap">
	...
	<div class="container">
		...
		<div class="contents">
            ...
            <!-- //서브타이틀 -->
            <form:form commandName="cs2000mVO" id="detailForm" name="detailForm" method="post" enctype="multipart/form-data">
				<input type="hidden" id="corpCd" name="corpCd" value="${cs2000mVO.corpCd}">
				<%-- <input type="hidden" id="ctrtNo" name="ctrtNo" value="${cs2000mVO.ctrtNo}"> --%>
				<input type="hidden" id="inTamt" name="inTamt" value="${cs2000mVO.inTamt}">
				<input type="hidden" id="inBamt" name="inBamt" value="${cs2000mVO.inBamt}">
            <!-- 견적정보 -->
            <table class="tbl_view">
                ...
                <tbody>
                    <tr>
                        <th scope="row">계약번호(변경불가)</th>
                        <td colspan="3">
                        	<input type="text" value="<c:out value="${cs2000mVO.ctrtCd }" />"  class="p40" id="ctrtCd" name="ctrtCd"/>
                        </td>
                    </tr>
                    <tr>    
                        <th scope="row"><i>*</i>계약일자</th>
                        <td>
                            <div class="date_sec">
                                <span>
                                    <input name="ctrtDt" id="datepicker01" placeholder="0000-00-00" value="<c:out value="${cs2000mVO.ctrtDt }" />" class="date required" title="게약일" />
                                    <label for="datepicker01" class="btn_cld">날짜검색</label>
                                </span>
                            </div>
                        </td>
                        <th scope="row" class="bl"><i>*</i>계약담당자</th>
                        <td>
                           	<input type="text" value="<c:out value="${cs2000mVO.empName }" />"  class="p30" id="empName" name="empName"/>
                           	<input type="hidden"  value="<c:out value="${cs2000mVO.empNo }" />"  class="p30" id="empNo" name="empNo"/>
                    ...
                    <tr>
                        ...
                        <td colspan="3">
							<input type="text" value="<c:out value="${cs2000mVO.opCd }" />"  class="p30" id="opCd" name="opCd"/>
                           	<input type="hidden"  value="<c:out value="${cs2000mVO.opNo }" />"  class="p30" id="opNo" name="opNo"/>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" value="<c:out value="${cs2000mVO.vendName }" />"  class="p80" id="vendName" name="vendName"/>
							<input type="hidden" value="<c:out value="${cs2000mVO.vendNo }" />"  class="p80" id="vendNo" name="vendNo"/>
                            <label for="modi-pop2" class="btn_sub dispNone">조회</label>
                        </td>
                        <th scope="row" class="bl"><i>*</i>계약분야</th>
                        <td>
                        	<!-- 계약분야 목록(공통코드) CM4000M - CM1360  -->
							<select id="ctrtCls" name="ctrtCls" class="p30">
								<option value="" <c:if test="${cs2000mVO.ctrtCls eq '' }">selected="selected"</c:if> >선택</option>
								<c:forEach var="result" items="${cm1360}">
									<option value="${result.cd}"<c:if test="${result.cd eq cs2000mVO.ctrtCls}">selected="selected"</c:if>>${result.cdName}</option>
								</c:forEach>
							</select>
						</td>	
					</tr>		
					<tr>
                        <th scope="row"><i>*</i>계약명</th>                        
                        <td colspan="3">
							<textarea class="type02 type03" id="ctrtName" name="ctrtName" placeholder="계약명을 입력하세요"><c:out value="${cs2000mVO.ctrtName }" /></textarea>
						</td>                        
                    </tr>
                    <tr>
                        <th scope="row"><i>*</i>계약내용</th>                        
                        <td colspan="3">
							<textarea class="type02 type03" id="ctrtCont" name="ctrtCont" placeholder="계약내용을 입력하세요"><c:out value="${cs2000mVO.ctrtCont }" /></textarea>
						</td>                        
                    </tr>
                    <tr>    
                        <th scope="row"><i>*</i>계약시작일자</th>
                        <td>
                            <div class="date_sec">
                                <span>
                                    <input name="ctrtStdt" id="datepicker02" placeholder="0000-00-00" value="<c:out value="${cs2000mVO.ctrtStdt }" />" class="date required" title="게약시작일" />
                                    <label for="datepicker02" class="btn_cld">날짜검색</label>
                                </span>
                            </div>
                        </td>
                        <th scope="row" class="bl"><i>*</i>계약종료일자</th>
                        <td>
                            <div class="date_sec">
                                <span>
                                    <input name="ctrtEndt" id="datepicker03" placeholder="0000-00-00" value="<c:out value="${cs2000mVO.ctrtEndt }" />" class="date required" title="게약종료일" />
                    ...
                    <tr>
                        ...
                        <td>
							<input type="text" value="<c:out value="${cs2000mVO.vmngName }" />"  class="p50" id="vmngName" name="vmngName" />
						</td>
						<th scope="row" class="bl">연락처</th>
                        <td>
							<input type="text" value="<c:out value="${cs2000mVO.vmnghpNo }" />"  class="p50" id="vmnghpNo" name="vmnghpNo" />
						</td>
					</tr>
					<tr>
                        <th scope="row">메일주소</th>
                        <td colspan="3">
							<input type="text" value="<c:out value="${cs2000mVO.vmngEmail }" />"  class="p30" id="vmngEmail" name="vmngEmail" />
						</td>
					</tr>
					 <tr>
                        <th scope="row"><i>*</i>연구비</th>
                        <td>
							<input type="text" value="<c:out value="${cs2000mVO.rsPay }" />"  class="p30 txt-r" id="rsPay" name="rsPay" onkeyup="fn_number(this);" onchange="fn_setVat()" />원
						</td>
						<th scope="row" class="bl">부가세</th>
                        <td>
							<input type="text" value="<c:out value="${cs2000mVO.rsPayvt }" />"  class="p30 txt-r" id="rsPayvt" name="rsPayvt" onkeyup="fn_number(this);"/>원
						</td>
					</tr>
					<tr>
                        <th scope="row">합계</th>
                        <td colspan="3">
							<input type="text" value="<c:out value="${cs2000mVO.rsTpay }" />"  class="p15 txt-r" id="rsTpay" name="rsTpay" onkeyup="fn_number(this);"/>원
						</td>
					</tr>	
					<tr>
                        <th scope="row">비고</th>                        
                        <td colspan="3">
							<textarea class="type02 type03" id="remk" name="remk"><c:out value="${cs2000mVO.remk }" /></textarea>
						</td>                        
                    </tr>
                    <tr>
                        <th scope="row">수금조건</th>
                        <td>
                        	<input type="text" value="<c:out value="${cs2000mVO.inCond }" />"  class="p40" id="inCond" name="inCond"/>
                        </td>
                        <th scope="row" class="bl">회신상태</th>
                        <td>
                        	<input type="radio" name="rtYn" id="rtYn1" value="Y"/>
						    <label for="rtYn1">회신확인</label>
						    <input type="radio" name="rtYn" id="rtYn2" value="N" checked="checked"/>
						    <label for="rtYn2">미확인</label>
                        	<%-- <input type="text" value="<c:out value="${cs2000mVO.rtYn }" />"  class="p40" id="rtYn" name="rtYn"/> --%>
                        </td>
                    </tr>
                    <tr>    
                        ...
                        <td>
                            <div class="date_sec">
                                <span>
                                    <input name="esDt" id="datepicker04" placeholder="0000-00-00" value="<c:out value="${cs2000mVO.esDt }" />" class="date required" title="이메일발송일" />
                        ...
                        <td>
                            <div class="date_sec">
                                <span>
                                    <input name="psDt" id="datepicker05" placeholder="0000-00-00" value="<c:out value="${cs2000mVO.psDt }" />" class="date required" title="우편발송일" />
                    ...
                    <tr>
						...
						<td colspan="3">
							...
							<select id="branchCd" name="branchCd" class="p15">
								<option value="" <c:if test="${cs2000mVO.branchCd eq '' }">selected="selected"</c:if> >선택</option>
								<c:forEach var="result" items="${branch}">
									<option value="${result.branchCd}"<c:if test="${result.branchCd eq cs2000mVO.branchCd}">selected="selected"</c:if>>${result.branchName}</option>
								</c:forEach>
							</select>	
						</td>
                    </tr>
                    <tr>
                        ...
                        <td colspan="3">
							<input type="text" value="<c:out value="${cs2000mVO.ctrtNo }" />"  class="p20" id="ctrtNo" name="ctrtNo" />
						</td>
					</tr>
                </tbody>
            </table>
		...
		<div class="modi-popup3" style="width:600px; height:300px; top:60%; left:40%;">
			...
			<table class="tbl_list">
				...
				<tbody id="stdBody3">
					<tr>
						<td colspan="6">견적명 또는 고객사명으로 검색해주세요.</td>
					</tr>
				</tbody>
			</table>
			<!-- //목록 -->
