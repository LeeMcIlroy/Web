...
<script type="text/javascript">
	...
	function fn_step(){
		if(confirm('대상자의 예약정보를 SMS로 발송합니다.\r\n저장하시겠습니까?')){
			...
			var receNo = $("#receNo").val();
			var rsNo = $("#rsNo").val();
			var rsjNo = $("#rsjNo").val();
			$.ajax({
				url: "<c:url value='${pageContext.request.contextPath }/qxsepmny/cmm/cmmAjaxSaveSmsMt.do'/>"
				, type: "post"
				, data: "corpCd="+corpCd+"&"+"rsNo="+rsNo+"&"+"rsjNo="+rsjNo+"&"+"receName="+receName+"&"+"receNo="+receNo+"&"+"recsDt="+recsDt+"&"+"sendmCls="+sendmCls+"&"+"resrDt="+resrDt+"&"+"resrHr="+resrHr+"&"+"resrMm="+resrMm+"&"+"cont="+cont+"&"+$("input[name=rsjSeq]:checked").serialize()
				, dataType:"json"
	...
	//회차별 예약문구 자동 완성
	function fn_autocomp() {
		var select_obj = '';
		var chkstr = '';
	    var mtseq = 0;
	    select_obj += '[HNB SRC]'+$("#receName").val()+'님 ';
	    $("input[name=rsjSeq]:checked").each(function (index) {
	    	if (index != 0) {
	        	select_obj += ', ';
	        }
	        mtseq += 1;
	        ...
	        if($("input[name=rsjSeq]:checked").length == 1){
	           	select_obj += chkstr.substr(0,17);
	   		}else{
	   			select_obj += mtseq + '차 '+chkstr.substr(0,17);
	   		}           
	    });
	    select_obj += '예약 되었습니다. 서울시 강남구 논현로 175길 38, 서원빌딩 3층 HNB 피부임상연구센터';
	    $("#cont").val(select_obj);
	}
...
<div class="pop_wrap">
	...
	<form:form commandName="rm2000mVO" id="detailForm" name="detailForm" method="post" enctype="multipart/form-data">
			...
			<input type="hidden" id="receNo" name="receNo" value="${rm2000mVO.receNo}">
			<input type="hidden" id="rsjNo" name="rsjNo" value="${rm2000mVO.rsjNo}">
			<input type="hidden" id="rsNo" name="rsNo" value="${rm2000mVO.rsNo}">
			<input type="hidden" id="receName" name="receName" value="${rm2000mVO.receName}">
		...
		<div class="hd_fix">
				<table class="tbl_list">
					...
					<tr>
						<td><input type="checkbox" name="rsjSeq" value="<c:out value='${result.resrDay }'/><c:out value='${result.rsNo }'/><c:out value='${result.rsjNo }'/><c:out value='${result.hpNo }'/>"/></td>
						...
						<td><c:out value="${result.resrDay}"/></td>
					...
					<c:if test="${resultList.size() == 0 }">
						<tr>
							<td class="nodata" colspan="7">선택한 예약정보가 없습니다.</td>
						</tr>
					</c:if>
					</tbody>
				</table>
		...
		<div class="btn_area">
			...
			<a href="#" onclick="fn_autocomp(); return false;">자동완성</a>
