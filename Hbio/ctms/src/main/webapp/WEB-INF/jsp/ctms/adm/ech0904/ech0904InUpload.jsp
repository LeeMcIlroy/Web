...
<script type="text/javascript">
	...
	function fn_uploadMemData(){
		...
		if(confirm("선택하신 파일을 업로드 하시겠습니까?")){
			$("#frm").attr("action", "<c:url value='/qxsepmny/ech0904/ech0904UploadMemData.do'/>").submit();
/* 			.attr("method", "post")
			.attr("enctype", "multipart/form-data") 
			.attr("action", "<c:url value='/qxsepmny/ech0904/ech0904UploadCsData.do'/>").submit(); */
		}
	}
	...
	function fn_saveMemData() {
		$("#frm").attr("method", "post").attr("action", "<c:url value='/qxsepmny/ech0904/ech0904SaveInData.do'/>").submit();
	}
	...
	<div class="container">
		...
		<div class="contents">
			...
			<jsp:include page="/WEB-INF/jsp/ctms/adm/inc/incPageTitle.jsp">
	            ...
	            <jsp:param name="dept2" value="입금관리(일괄등록)"/>
           	</jsp:include>
			<!-- //타이틀 -->
			<!-- 서브타이틀 -->
								...
								<tbody>
									<tr>
										...
										<td>
											<c:choose>
												...
												<c:otherwise>
													<c:out value="${fn:length(insertMemList)}" />건      <span>‘저장’버튼을 클릭하면 신규 입금정보만 등록됩니다.</span>
												</c:otherwise>
											</c:choose>
										</td>
									</tr>
									...
									<c:if test="${!empty excelDupMemList}" >
										...
										<tr>
											...
											<td>
												요청 <c:out value="${result.vendNo}"/>-<c:out value="${result.inDt}"/>-<c:out value="${result.inAmt}"/>
												...
												<c:forEach var="dup" items="${result.dupMemList}">
													중복 <c:out value="${dup.vendNo}"/>-<c:out value="${dup.inDt}"/>-<c:out value="${dup.inAmt}"/>
