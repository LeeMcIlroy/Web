...
<script type="text/javascript">
	...
	function fn_excelForm(){
		$("#frm").attr("action","<c:url value='/qxsepmny/ech0101/uploadFormDownload.do'/>").submit();
	}
	...
	function fn_uploadMemData(){
		...
		if(confirm("선택하신 파일을 업로드 하시겠습니까?")){
			$("#frm").attr("action", "<c:url value='/qxsepmny/ech0101/ech0101UploadMemData.do'/>").submit();
/* 			.attr("method", "post")
			.attr("enctype", "multipart/form-data") 
			.attr("action", "<c:url value='/qxsepmny/ech0101/ech0101UploadMemData.do'/>").submit(); */
		}
	}
	...
	function fn_saveMemData() {
		$("#frm").attr("method", "post").attr("action", "<c:url value='/qxsepmny/ech0101/ech0101SaveRsData.do'/>").submit();
	}
	...
	<div class="container">
		...
		<div class="contents">
			...
			<jsp:include page="/WEB-INF/jsp/ctms/adm/inc/incPageTitle.jsp">
	            ...
	            <jsp:param name="dept2" value="연구관리(일괄등록)"/>
           	</jsp:include>
			<!-- //타이틀 -->
			<!-- 서브타이틀 -->
								...
								<tbody>
									<tr>
										...
										<td>
											...
											<button type="button" class="btn-download-file" onclick="fn_filedownload('1285', 'UPLS'); return false;">양식다운로드</button>
										</td>
									</tr>
								</tbody>
							</table>
								...
								<tbody>
									<tr>
										...
										<td>
											<c:choose>
												...
												<c:otherwise>
													<c:out value="${fn:length(insertMemList)}" />건      <span>‘저장’버튼을 클릭하면 신규연구과제만 등록됩니다.</span>
												</c:otherwise>
											</c:choose>
										</td>
									</tr>
									...
									<c:if test="${!empty excelDupMemList}" >
										...
										<tr>
											...
											<td>
												요청 <c:out value="${result.rsNo1}"/><c:out value="${result.rsNo2}"/>-<c:out value="${result.rsNo3}"/><c:out value="${result.rsNo4}" />-<c:out value="${result.rsNo5}" /><c:out value="${result.rsNo6}" />-<c:out value="${result.rsNo7}" />
												...
												<c:forEach var="dup" items="${result.dupMemList}">
													중복 <c:out value="${dup.rsNo1}" /><c:out value="${dup.rsNo2}" />-<c:out value="${dup.rsNo3}" /><c:out value="${dup.rsNo4}" />-<c:out value="${dup.rsNo5}" /><c:out value="${dup.rsNo6}" />-<c:out value="${dup.rsNo7}" />
													<%-- <c:out value="${dup.name}" /> 
													<c:out value="${dup.eMail}" />
													<c:out value="${dup.phoneNo}" /> --%>
													<%-- <c:out value="${dup.core eq 'Y' ? '핵심고객, ' : ''}" />
													<c:out value="${dup.corr eq 'Y' ? '현지통신원, ' : ''}" />
													<c:out value="${dup.reporter eq 'Y' ? 'KREI리포터,' : ''}" />
													<c:out value="${dup.tot eq 'Y' ? '통합배부처, ' : ''}" />
													<c:out value="${dup.payb eq 'Y' ? '유료도서회원, ' : ''}" /> --%>
													<br/>
												</c:forEach>
											</td>	
											</tr>
