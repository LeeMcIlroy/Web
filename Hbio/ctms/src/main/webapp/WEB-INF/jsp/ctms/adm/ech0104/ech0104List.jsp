...
<script type="text/javascript">
	...
	function fn_view(corpCd, rsNo, rvNo){		
	 	...
	 	$("#rvNo").val(rvNo);
	 	if(rvNo =='') {
	 		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0104/ech0104Modify.do'/>").submit();
	 	}else{
	 		$("#listForm").attr("action","<c:url value='/qxsepmny/ech0104/ech0104View.do'/>").submit();	
	 	}
	}
	...
	function fn_list(pageNo){
		...
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0104/ech0104List.do'/>").submit();
	}
	...
	function fn_excelDown(){
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0104/ech0104Excel.do'/>").submit();
	}
			...
			<table class="tbl_list tr_link">
				...
				<thead>
					<tr>
						...
						<th scope="col">계획승인일</th>
						<th scope="col">변경승인일</th>
						<th scope="col">결과승인일</th>
					</tr>
				</thead>
				<tbody>
				<c:forEach var="result" items="${resultList}" varStatus="status">
					<tr onclick="fn_view('<c:out value='${result.corpCd}'/>','<c:out value='${result.rsNo}'/>','<c:out value='${result.rvNo}'/>'); return false;">					
						...
						<td><c:out value="${result.rvDt }"/></td>
						<td><c:out value="${result.rvNo }"/></td>
						<td><c:out value="${result.rvdocClsNm }"/></td>
						<td><c:out value="${result.rvStNm }"/></td>
						<td><c:out value="${result.rvRsNm }"/></td>
						<td><c:out value="${result.plrvDt }"/></td>
						<td><c:out value="${result.chrvDt }"/></td>
						<td><c:out value="${result.rsrvDt }"/></td>
					</tr>
				</c:forEach>
				<c:if test="${resultList.size() == 0 }">
					<tr>
						<td class="nodata" colspan="15">IRB심의 정보가 없습니다.</td>
					</tr>
				</c:if>
				</tbody>
			</table>
