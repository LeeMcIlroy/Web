...
<script type="text/javascript">
	...
	function fn_list(pageNo){
		...
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0804/ech0804List.do'/>").submit();
	}
	//작업로그 엑셀다운로드
	function fn_excelDown(){
		if(confirm('출력하는 데이터건수에 따라 시간이 지체될 수 있습니다. \r\n진행하시겠습니까?')){
			$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0804/ech0804Excel.do'/>").submit();
		}
	}
	...
	<div class="container">
		...
		<div class="contents">
			...
			<jsp:include page="/WEB-INF/jsp/ctms/adm/inc/incPageTitle.jsp">
	            ...
	            <jsp:param name="dept2" value="작업로그"/>
           	</jsp:include>
			<!-- //타이틀 -->
			<!-- 검색영역 -->
					...
					<p>검색어</p>
						<span>
							<select name="searchCondition3" id="searchCondition3" onchange="fn_resetWord(); return false;">
								...
								<option value="2" <c:if test="${searchVO.searchCondition3  eq '2' }">selected="selected"</c:if> >작업내용</option>
								<option value="3" <c:if test="${searchVO.searchCondition3  eq '3' }">selected="selected"</c:if> >작업자</option>
			...
			<table class="tbl_list tr_link">
				...
				<tbody>
					...
					<c:if test="${resultList.size() == 0 }">
						<tr>
							<td class="nodata" colspan="8">작업로그 정보가 없습니다.</td>
						</tr>
					</c:if>	
				</tbody>
			</table>
