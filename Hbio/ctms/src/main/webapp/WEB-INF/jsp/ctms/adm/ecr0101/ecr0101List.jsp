...
<script type="text/javascript">
	...
	function fn_view(corpCd, rsNo){		
	 	$("#corpCd").val(corpCd);
	 	$("#rsNo").val(rsNo);
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ecr0101/ecr0101View.do'/>").submit();
	}
	...
	//CRF pdf 합치기
	function fn_pdfMergeRsNo(rsNo, mkCls){
		//alert(rsNo);
		//alert(mkCls);
		if(confirm('선택하신 연구과제의 CRF 파일이 생성됩니다(모든 연구대상자) \r\n생성하시겠습니까?')){		
			$.ajax({
				type: "POST"
		        , url:  "<c:url value='/qxsepmny/ecr0101/pdfMergeRsNo.do'/>"    
		        , data: {  
		    	    "rsNo"  : rsNo,
		    	    "mkCls"  : mkCls,
		        }
	...
	function fn_reqMkFin(corpCd, rsNo){
		if(confirm('연구종료확인서 승인을 요청합니다. \r\n진행하시겠습니까?')){
			//var corpCd = $("#corpCd").val();
			//var rsNo = $("#rsNo").val();			
			//var step1 = $("#step1").val();
			//var step2 = $("#step2").val();
			//alert(rsNo);
			$.ajax({
				url: "<c:url value='${pageContext.request.contextPath }/qxsepmny/ecr0101/ecr0101AjaxReqMkFin.do'/>"
				, type: "post"
		...
		<div class="contents">
			...
			<table class="tbl_list tr_link">
				...
				<c:forEach var="result" items="${resultList}" varStatus="status">
						...
						<td onclick="fn_view('<c:out value='${result.corpCd}'/>','<c:out value='${result.rsNo}'/>'); return false;" style="text-decoration: underline;">
							<c:choose>
                          		<c:when test="${result.dataLockYn eq 'N'}">
									<c:out value="${result.rsName }"/>
								</c:when>
								<c:when test="${result.dataLockYn eq 'Y'}">	
									<c:out value="${result.rsName }"/><span style="color:red;">(Data Locked)</span>
								</c:when>
							</c:choose>	
						</td>
						<%-- <td>
						...
						<td>
							<c:choose>
                          		<c:when test="${result.mkCls eq '1000' }">임상시험</c:when>
                          		<c:when test="${result.mkCls eq '1010' }">
                          			<a onclick="fn_reqMkFin('<c:out value="${result.corpCd }"/>','<c:out value="${result.rsNo }"/>');" class="btn_sub">승인요청</a>
                          		</c:when>
                          		<c:when test="${result.mkCls eq '1020' }"><span style="color:red;">승인대기</span></c:when>
                          		<c:when test="${result.mkCls eq '1030' }"><span style="color:blue;">승인</span></c:when>
                        	</c:choose>
						</td>
						<td>
							<%-- <c:choose>
                        	...
                        	<c:choose>
                          		...
                          		<c:when test="${result.mkCls eq '1000' }"><span style="color:red;">미작성</span></c:when>
                          		<c:when test="${result.mkCls eq '1010' }"><span style="color:red;">미작성</span></c:when>
                          		<c:when test="${result.mkCls eq '1020' }"><span style="color:red;">승인대기</span></c:when>
                          		<c:when test="${result.mkCls eq '1030' }">
                          			<c:choose>
                          				<c:when test="${result.dataLockYn eq 'N'}">
											<a onclick="fn_pdfMergeRsNo('<c:out value="${result.rsNo }"/>');" class="btn_sub">CRF생성(연구)</a>
										</c:when>
									</c:choose>
									<!-- CRF(연구) 다운로드,미리보기 -->
				...
				<c:if test="${resultList.size() == 0 }">
					<tr>
						<td class="nodata" colspan="6">CRF작성 정보가 없습니다.</td>
					</tr>
				</c:if>
				</tbody>
			</table>
