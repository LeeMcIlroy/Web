...
<script type="text/javascript">
	...
	//고객사관리 상세보기 화면
	function fn_view(corpCd, vendNo){		
	 	$("#corpCd").val(corpCd);
	 	$("#vendNo").val(vendNo);
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0704/ech0704View.do'/>").submit();
	}
	...
	//고객사관리 등록화면
	function fn_regist(){
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0704/ech0704Modify.do'/>").submit();
	}
	
	//고객사관리 엑셀다운로드
	function fn_excelDown(){
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0704/ech0704Excel.do'/>").submit();
	}
</script>
<body>
<form:form commandName="searchVO" id="listForm" name="listForm">
<input type="hidden" id="corpCd" name="corpCd"/>
<input type="hidden" id="vendNo" name="vendNo"/>
<!-- wrap -->
<div class="wrap">
		...
		<div class="contents">
			...
			<div class="srch_area">
				<ul>
                    ...
                    <li>
                        ...
                        <span class="type02">
                            <input type="text" name="searchWord" class="input-data"  placeholder="회사명, 대표자명을 입력하세요" />
                        </span>
					</li>
				</ul>
				<!-- 조회버튼 -->
			...
			<table class="tbl_list tr_link">
				...
				<c:forEach var="result" items="${resultList}" varStatus="status">			
					<tr onclick="fn_view('<c:out value='${result.corpCd }'/>', '<c:out value='${result.vendNo }'/>'); return false;">
						<td><c:out value="${paginationInfo.totalRecordCount+1 - ((searchVO.pageIndex-1) * paginationInfo.recordCountPerPage + status.count)}"/></td>
						<td><c:out value="${result.vendName }"/>
						<td><c:out value="${result.excutName}"/></td>
						<td><c:out value="${result.dispbregRsno}"/></td>
						<td><c:out value="${result.telno}"/></td>
						<td><c:out value="${result.faxno}"/></td>
						<td><c:out value="${result.mngName}"/></td>
						<td><c:out value="${result.mngEmail}"/></td>
				...
				<c:if test="${resultList.size() == 0 }">
					<tr>
						<td class="nodata" colspan="11">고객사 정보가 없습니다.</td>
					</tr>
				</c:if>				
				</tbody>
			</table>
