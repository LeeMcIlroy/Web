...
<script type="text/javascript">
	...
	function fn_view(corpCd, rsNo){
		$("#corpCd").val(corpCd);
	 	$("#rsNo").val(rsNo);
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0402/ech0402View.do'/>").submit();
	}

	//연구과제관리 목록으로
	function fn_list(pageNo){
		$("#pageIndex").val(pageNo);
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0402/ech0402List.do'/>").submit();
	}
	
	//연구과제관리 목록 엑셀다운로드
	function fn_excelDown(){
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0402/ech0402Excel.do'/>").submit();
	}
	...
	<div class="container">
		...
		<div class="contents">
			...
			<input type="hidden" id="corpCd" name="corpCd"/>
				...
				<table class="tbl_list tr_link" style="width:1400px">
					<colgroup>
						<col style="width:5%" />
						<col style="width:10%" />
						<col style="width:6%" />
					...
					<thead>
						<tr>
	                        ...
	                        <th scope="col">계획승인</th>
	                        <th scope="col">변경승인</th>
	                        <th scope="col">종료승인</th>
						</tr>
					</thead>
					<tbody>
						<c:forEach items="${resultList }" var="result" varStatus="status">
							<tr>
		                        ...
		                        <td><c:out value="${result.prrvDt }"/></td>
						...
						<c:if test="${resultList.size() == 0 }">
							<tr>
								<td class="nodata" colspan="15">연구 정보가 없습니다.</td>
							</tr>
						</c:if>
					</tbody>
				</table>
