...
<script type="text/javascript">
	...
	function fn_step(){
		if(confirm('CRF단계 간편정보를 저장합니다.\r\n저장하시겠습니까?')){
			//필수정보
			var cdName = $("#cdName").val();
			if(cdName==''){
				alert('CRF단계명을 입력해주세요')
				$('#cdName').focus();
				return;
			}
			...
			$.ajax({
				url: "<c:url value='${pageContext.request.contextPath }/qxsepmny/ech0211/ech0211AjaxSaveVend.do'/>"
				, type: "post"
				, data: "cdName="+cdName
				, dataType:"json"
...
<div class="pop_wrap">
	...
	<form:form commandName="cm4000mVO" id="detailForm" name="detailForm" method="post" enctype="multipart/form-data">
			...
			<input type="hidden" id="cdCls" name="cdCls" value="${cm4000mVO.cdCls eq ''?cdCls:cm4000mVO.cdCls}">
		...
		<table class="tbl_view mb01">
			...
			<tbody>
				...
				<%-- <tr>
					...
					<td>
						<input type	="text" value="<c:out value="${cm4000mVO.vendSm }" />"  class="input-data" id="vendSm" name="vendSm"/>
					</td>	
				</tr>
				<tr>
					<th scope="row"><i>*</i>대표자명</th>
					<td>
						<input type	="text" value="<c:out value="${cm4000mVO.excutName }" />"  class="input-data" id="excutName" name="excutName"/>
					</td>	
				</tr>
				<tr>
					...
					<td>
						...
						<select id="vendCls" name=vendCls>
							<c:forEach var="result" items="${cm1010}">
								<option value="${result.cd}"<c:if test="${result.cd eq cm4000mVO.vendCls}">selected="selected"</c:if>>${result.cdName}</option>
							</c:forEach>
						</select>
					</td>
				</tr> --%>
		...
		<%-- <table class="tbl_view mb01">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						<input type="text" value="<c:out value="${cm4000mVO.mngName }" />" class="input_data" id="mngName" name="mngName"/>
					</td>
				</tr>
				<tr>	
					<th scope="row">부서/직위</th>
					<td>
						<input type="text" value="<c:out value="${cm4000mVO.mngOrg }" />" class="input_data" id="mngOrg" name="mngOrg"/>
					</td>
				</tr>
			</tbody>
		</table>
