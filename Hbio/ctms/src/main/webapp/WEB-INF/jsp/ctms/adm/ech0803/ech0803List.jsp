...
<script type="text/javascript">
	...
	function fn_view(corpCd, empNo){		
	 	$("#corpCd").val(corpCd);
	 	$("#empNo").val(empNo);
	 	$("#listForm").attr("action","<c:url value='/qxsepmny/ech0803/ech0803View.do'/>").submit();
	}
	...
	function fn_list(pageNo){
		...
		$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0803/ech0803List.do'/>").submit();
	}
		
	//개인정보처리로그 엑셀다운로드
	function fn_excelDown(){
		if(confirm('출력하는 데이터건수에 따라 시간이 지체될 수 있습니다. \r\n진행하시겠습니까?')) {
			$("#listForm").attr("action", "<c:url value='/qxsepmny/ech0803/ech0803Excel.do'/>").submit();
		}
	}
	...
	<div class="container">
		...
		<div class="contents">
			...
			<jsp:include page="/WEB-INF/jsp/ctms/adm/inc/incPageTitle.jsp">
	            ...
	            <jsp:param name="dept2" value="개인정보처리로그"/>
           	</jsp:include>
			<!-- //타이틀 -->
			<!-- 검색영역 -->
					...
					<p>검색어</p>
						<span>
							<select name="searchCondition3" id="searchCondition3" onchange="fn_resetWord(); return false;">
								...
								<option value="3" <c:if test="${searchVO.searchCondition3  eq '3' }">selected="selected"</c:if> >대상자</option>
								<option value="4" <c:if test="${searchVO.searchCondition3  eq '4' }">selected="selected"</c:if> >지사</option>
							</select>
						</span>
                        <span class="type02">
                            <input type="text" name="searchWord" id="searchWord" class="input-data" value="<c:out value="${searchVO.searchWord }" />" placeholder="이름을 입력하세요" />
                        </span>
					</li>
				</ul>
				<!-- 조회버튼 -->
				...
				<tbody>
					<c:forEach var="result" items="${resultList}" varStatus="status">
						<tr id="contextMenu">
							...
							<td><c:out value="${result.mbCd}"/></td>								
	                        <td class="keyv"><c:out value="${result.mbName}"/></td>
							<td class="keyv"><c:out value="${result.logCont}"/></td>
					...
					<c:if test="${resultList.size() == 0 }">
						<tr>
							<td class="nodata" colspan="9">개인정보처리 로그 정보가 없습니다.</td>
						</tr>
					</c:if>	
				</tbody>
			</table>
