...
namespace HALLA_PM.Controllers.SiteMngHome
{
    ...
    [RoutePrefix("SiteMngHome/Performance/Pal")]
    public class PmPalController : SiteMngBaseController
    {
        [Route("List")]
        public ActionResult List(Search search)
        {
            ...
            model.list = pmPalRepo.selectList(search);
            model.UnionList = orgUnionRepo.selectList(new { });
            model.OrgCpyList = orgCompanyRepo.selectList(null);
            ViewBag.Search = search;

            return View("~/Views/SiteMngHome/Performance/Pal/List.cshtml", model);
        }

        [Route("Write")]
        public ActionResult Write(PmPal entity, Search search)
        {
            ...
            ViewBag.Search = search;
            return View("~/Views/SiteMngHome/Performance/Pal/Write.cshtml", model);
        }
        ...
        public ActionResult CalculationAction(PmPalAdmin entity, Search search)
        {
            #region
        ...
        [Route("View")]
        public ActionResult view(PmPal entity, Search search)
        {
            if (Request.RequestType == "GET") return RedirectToAction("List");
            ...
            var selectListAnalysisMonthlyContent = pmPalBusinessRepo.selectListAnalysisMonthlyContent(new
            {
                YearlyYear = pmPal.PalYear,
                Monthly = pmPal.Monthly,
                //OrgCompanySeq = pmPal.Seq,
                OrgCompanySeq = pmPal.OrgCompanySeq,
            ...
            ViewBag.Search = search;
            return View("~/Views/SiteMngHome/Performance/Pal/View.cshtml", model);
        }

        [Route("Edit")]
        public ActionResult Edit(PmPal entity, Search search)
        {
            if (Request.RequestType == "GET") return RedirectToAction("List");
            ...
            ViewBag.Search = search;
            return View("~/Views/SiteMngHome/Performance/Pal/Edit.cshtml", model);
        }



        [Route("Edit2")]
        public ActionResult Edit2(PmPal entity, Search search)
        {
            if (Request.RequestType == "GET") return RedirectToAction("List");
            ...
            ViewBag.Search = search;
            return View("~/Views/SiteMngHome/Performance/Pal/Edit2.cshtml", model);
        }


        [Route("Write2")]
        public ActionResult Write2(PmPal entity, Search search)
        {
            if (Request.RequestType == "GET") return RedirectToAction("List");
            ...
            ViewBag.Search = search;
            return View("~/Views/SiteMngHome/Performance/Pal/Write2.cshtml", model);
        }

        [Route("Edit_Action")]
        public ActionResult EditAction(PmPalAdmin entity, Search search)
        {
            #region
            Dictionary<string, object> postData = new Dictionary<string, object>();
            postData.Add("Seq", entity.Seq);
        ...
        public ActionResult RegistStatusChange(PmPal entity, Search search)
        {
            if (Request.RequestType == "GET") return RedirectToAction("List");
        ...
        [Route("Excel_Down2")]
        public ActionResult ExcelDown2(PmPal entity)
        {
            if (Request.RequestType == "GET") return RedirectToAction("List");
            ...
            var orgCompanyName = orgCompanyRepo.selectOne(new { Seq = entity.OrgCompanySeq });
            var pBusinessList = pmPalBusinessRepo.selectListExcel(new { PmPalSeq = entity.Seq });
            var pPalSummaryList = pmPalSummaryRepo.selectListExcel(new { PmPalSeq = entity.Seq });
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
               ...
               // ws.Column(1).Hide();
                ...
                wt.Cell(rows, ++cells).SetValue("월").Style = wtHeaderStyle;
                wt.Cell(rows, ++cells).SetValue("타입").Style = wtHeaderStyle;
                wt.Cell(rows, ++cells).SetValue("월누계").Style = wtHeaderStyle;
                ...
                if (pPalSummaryList.Count() == 0)
                {
                    for (int i = 0; i < 3; i++)
                    {
                        var strMonthType = (i + 1) + "0";
                        rows++;
                        cells = 1;
                        ...
                        wt.Cell(rows, cells).SetValue(pmPal.PalYear).Style = wtHeaderStyle.Fill.SetBackgroundColor(XLColor.NoColor);
                        wt.Cell(rows, ++cells).SetValue(pmPal.Monthly).Style = wtHeaderStyle.Fill.SetBackgroundColor(XLColor.NoColor);
                        wt.Cell(rows, ++cells).SetValue(strMonthType).Style = wtHeaderStyle.Fill.SetBackgroundColor(XLColor.NoColor);
                        wt.Cell(rows, ++cells).SetValue(Define.MONTHLY_TYPE.GetValue(int.Parse(strMonthType))).Style = wtHeaderStyle.Fill.SetBackgroundColor(XLColor.NoColor);
                ...
                foreach (var item in pPalSummaryList)
                {
                    rows++;
                    cells = 1;
                    // ws.Cell(rows, cells).SetValue(item.PmPalSeq).Style = wsHeaderStyle.Fill.SetBackgroundColor(XLColor.NoColor);
                    ...
                    wt.Cell(rows, cells).SetValue(item.PalYear).Style = wtHeaderStyle.Fill.SetBackgroundColor(XLColor.NoColor);
                    wt.Cell(rows, ++cells).SetValue(item.Monthly).Style = wtHeaderStyle.Fill.SetBackgroundColor(XLColor.NoColor);
                    wt.Cell(rows, ++cells).SetValue(item.MonthlyType).Style = wtHeaderStyle.Fill.SetBackgroundColor(XLColor.NoColor);
                    wt.Cell(rows, ++cells).SetValue(Define.MONTHLY_TYPE.GetValue(int.Parse(item.MonthlyType))).Style = wtHeaderStyle.Fill.SetBackgroundColor(XLColor.NoColor);
        ...
        /// <returns></returns>
        [Route("Excel_Upload2")]
        public ActionResult ExcelUpload2(PmPal entity, List<int> PmPalBusinessSeq, HttpPostedFileBase ExcelFile)
        {
            // 엑셀파일 검증 후
            // 문제가 없다면 엑셀 파일 데이터에 입력
            // 저장 완료되면 엑셀 로그에 데이터 남기고 해당 파일 저장
            ...
            if (ExcelFile.ContentLength != 0)
            {
                ...
                try
                {
                    ...
                    foreach (DataRow item in dt.Rows)
                    {
                        ...
                        //string companyName = item.ItemArray.GetValue(2).ToString();
                        // string businessSeq = intListSeq[i].ToString();
                        //string businessName = item.ItemArray.GetValue(4).ToString();
                        //string PalYear = item.ItemArray.GetValue(5).ToString();
                        //string Monthly = item.ItemArray.GetValue(6).ToString();
                        // string MonthlyType = item.ItemArray.GetValue(7).ToString();
                ...
                catch (Exception e)
                {
                    ...
                    //LogUtil.MngError(e.ToString());
                    result = false;
                    resultMsg = Message.MSG_007_E + " " + e.Message.ToString();
                    return Json(new { success = result, resultMsg = resultMsg }, JsonRequestBehavior.AllowGet);
                }
                ...
                eFileList.EmpNo = SessionManager.GetAdminSession().insaUserV.empNo;
                eFileList.AttachTableName = "PM_PAL";
                eFileList.AttachTableSeq = entity.Seq;
                eFileList.FileInputName = "UploadExcel";
                eFileList.FileOrgName = file["ORIGINAL_FILE_NAME"];
                eFileList.FilePath = filePath;
                eFileList.FileStoredName = file["STORED_FILE_NAME"];
        ...
        /// <returns></returns>
        [Route("Excel_Upload3")]
        public ActionResult ExcelUpload3(PmPal entity, List<int> PmPalBusinessSeq, HttpPostedFileBase ExcelFile)
        {
            // 엑셀파일 검증 후
            // 문제가 없다면 엑셀 파일 데이터에 입력
            // 저장 완료되면 엑셀 로그에 데이터 남기고 해당 파일 저장
            ...
            if (ExcelFile.ContentLength != 0)
            {
                ...
                List<PmPalBusiness> pBusinessList = new List<PmPalBusiness>();
                List<PmPalSummary> pPalSummaryList = new List<PmPalSummary>();
                ...
                try
                {
                    if (dt.Tables["손익"] == null || dt.Tables["손익"].Rows.Count == 0)
                    {
                        // 오류시 파일 삭제
                        fileUtil.FileDelete(filePath, file["STORED_FILE_NAME"]);
                        result = false;
                    ...
                    if (dt.Tables["손익연결"] == null || dt.Tables["손익연결"].Rows.Count == 0)
                    {
                        // 오류시 파일 삭제
                        fileUtil.FileDelete(filePath, file["STORED_FILE_NAME"]);
                        result = false;
                    ...
                    int j = 0;
                    foreach (DataRow item in dt.Tables["손익"].Rows)
                    {
                        //string parentSeq = "0";
                        string excelSeq = PmPalBusinessSeq[j].ToString();
                    ...
                    j = 0;
                    foreach (DataRow item in dt.Tables["손익연결"].Rows)
                    {
                        //string parentSeq = "0";
                        //string excelSeq = PmPalBusinessSeq[j].ToString();
                        string montyType = item.ItemArray.GetValue(2).ToString();
                        ...
                        string netProfitTerm = item.ItemArray.GetValue(7).ToString();
                        ...
                        var pPalSummary = new PmPalSummary();
                       // pPalSummary.Seq = Convert.ToInt32(excelSeq);
                        pPalSummary.PmPalSeq = entity.Seq;
                        pPalSummary.MonthlyType = montyType;
                        pPalSummary.Sales = Convert.ToDecimal(sales);
                        pPalSummary.Ebit = Convert.ToDecimal(ebit);
                        pPalSummary.Pbt = Convert.ToDecimal(pbt);
                        pPalSummary.NetProfitTerm = Convert.ToDecimal(netProfitTerm);

                        pPalSummaryList.Add(pPalSummary);
                        j += 1;
                    }
                }
                catch (Exception e)
                ...
                for (int i = 0; i < pPalSummaryList.Count(); i++)
                {
                    PmPalSummary pSum = new PmPalSummary();
                    pSum.PmPalSeq = pPalSummaryList[i].PmPalSeq;
                    pSum.MonthlyType = pPalSummaryList[i].MonthlyType;
                    pSum.Sales = pPalSummaryList[i].Sales;
                    pSum.Ebit = pPalSummaryList[i].Ebit;
                    pSum.Pbt = pPalSummaryList[i].Pbt;
                    pSum.NetProfitTerm = pPalSummaryList[i].NetProfitTerm;
                    pmPalSummaryRepo.insert2(pSum);
                }
