...
{
    public class PlanYearBsSummaryExRepo : DbCon, IPlanYearBsSummaryExRepo
    {
        ...
        public int count(object param, string where)
        {
            using (IDbConnection con = GetHallaDb())
            {
                ...
                {
                    con.Open();
                    string query = " SELECT COUNT(*) FROM PLAN_YEAR_BS_SUMMARY_EX " + where;
        ...
        public int delete(object key)
        {
            using (IDbConnection con = GetHallaDb())
            {
                ...
                {
                    con.Open();
                    string query = " DELETE PLAN_YEAR_BS_SUMMARY_EX " +
        ...
        public int insert(PlanYearBsSummaryEx entity)
        {
            using (IDbConnection con = GetHallaDb())
            {
                try
                {
                    con.Open();
                    string query = @" INSERT INTO PLAN_YEAR_BS_SUMMARY_EX ( 
                             PLAN_YEAR_BS_SEQ 
                            , YEARLY_YEAR
                            , ASSETS
                            , LIABILITIES
        ...
        public int insertCum(PlanYearBsSummaryEx entity)
        {
            using (IDbConnection con = GetHallaDb())
            {
                try
...
INSERT INTO PLAN_YEAR_BS_SUMMARY_EX
...
LEFT OUTER JOIN PLAN_YEAR_BS_BSHEET_EX			B ON A.SEQ = B.PLAN_YEAR_BS_SEQ AND (B.MONTHLY = '12' OR B.MONTHLY = '99')
LEFT OUTER JOIN PLAN_YEAR_BS_ROIC_EX			C ON A.SEQ = C.PLAN_YEAR_BS_SEQ AND B.YEARLY_YEAR = C.YEARLY_YEAR AND B.MONTHLY = C.MONTHLY
LEFT OUTER JOIN PLAN_YEAR_BS_W_CAPITAL_EX		D ON A.SEQ = D.PLAN_YEAR_BS_SEQ AND B.YEARLY_YEAR = D.YEARLY_YEAR AND B.MONTHLY = D.MONTHLY
LEFT OUTER JOIN PLAN_YEAR_BS_W_CAPITAL_REG_EX	E ON A.SEQ = E.PLAN_YEAR_BS_SEQ AND B.YEARLY_YEAR = E.YEARLY_YEAR AND D.MONTHLY = E.MONTHLY
WHERE	1 = 1
...
SELECT CAST(SCOPE_IDENTITY() as int)";
        ...
        public int save(PlanYearBsSummaryEx entity)
        {
            throw new NotImplementedException();
        }

        public IEnumerable<PlanYearBsSummaryEx> selectList(object param)
        {
            using (IDbConnection con = GetHallaDb())
            {
                try
                {
                    ...
                    string query = @" SELECT *
                                      FROM	PLAN_YEAR_BS_SUMMARY_EX
                                      WHERE	PLAN_YEAR_BS_SEQ = @PlanYearBsSeq ";

                    return con.Query<PlanYearBsSummaryEx>(query, param).ToList();
        ...
        public IEnumerable<PlanYearBsSummaryExExp> selectListPnThisYear(object param)
        {
            using (IDbConnection con = GetHallaDb())
            {
                try
...
SELECT	B.YEARLY_YEAR
		...
		, C.COMPANY_NAME
FROM	PLAN_YEAR_BS_EX							A
LEFT OUTER JOIN PLAN_YEAR_BS_SUMMARY_EX			B ON A.SEQ = B.PLAN_YEAR_BS_SEQ
...
";
                    ...
                    return con.Query<PlanYearBsSummaryExExp>(query, param).ToList();
                }
                catch (Exception e)
                {
                    LogUtil.MngError(e.ToString());
        ...
        public PlanYearBsSummaryEx selectOne(object param)
        {
            throw new NotImplementedException();
        }

        public PlanYearBsSummaryEx selectOneYear(object param)
        {
            using (IDbConnection con = GetHallaDb())
            {
                try
...
";
                    return con.Query<PlanYearBsSummaryEx>(query, param).FirstOrDefault();
                }
                catch (Exception e)
                {
                    LogUtil.MngError(e.ToString());
        ...
        public int update(PlanYearBsSummaryEx entity)
        {
            using (IDbConnection con = GetHallaDb())
            {
                try
                {
                    con.Open();
                    string query = " UPDATE PLAN_YEAR_BS_SUMMARY_EX SET " +
                        // 이부분은 수정하여 사용하세요. 
