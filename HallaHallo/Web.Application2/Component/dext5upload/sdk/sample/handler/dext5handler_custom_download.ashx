...
public class dext5handler : IHttpHandler
{
    ...
    {
        /**************** Custom Value(AddUploadedFile 5번째 파라미터)를 이용하여 다운로드 받을 파일의 경로 설정하는 부분 입니다. *****************/
        ...
        char m_PathChar = System.IO.Path.DirectorySeparatorChar;
        ...
        for (int i = 0; i < downloadCustomValue.Length; i++)
        {
            downloadCustomValue[i] = HttpUtility.UrlDecode(downloadCustomValue[i], System.Text.Encoding.UTF8);
            // Custom Value(AddUploadedFile 5번째 파라미터)를 이용하여 다운로드 받을 파일의 경로 설정
            switch (downloadCustomValue[i])
            {
                case "CustomValue1":
                    downloadFilePath[i] = System.IO.Path.Combine(TempPath, @"images" + m_PathChar + "Panorama" + m_PathChar + "ViewPhotos.jpg");
                    break;
                case "CustomValue2":
                    downloadFilePath[i] = System.IO.Path.Combine(TempPath, @"images" + m_PathChar + "Scenery" + m_PathChar + "image" + m_PathChar + "CreativeImages.bmp");
                    break;
            }
        }
