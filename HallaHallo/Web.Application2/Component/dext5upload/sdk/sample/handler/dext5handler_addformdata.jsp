<%@ page contentType="text/html;charset=utf-8"%><%
	...
	try {
		...
		if (FormDataValue != null && FormDataValue.length > 0) {
			...
			for(int i=0; i<FormDataValue.length; i++){
				FormDataValue[i] = java.net.URLDecoder.decode(FormDataValue[i], "UTF-8");
	...
	event.addOpenDownloadPathChangeEventListenerEx(new Raonwiz.Dext5.OpenDownloadPathChangeEventListenerEx() {
		public void OpenDownloadPathChangeEventEx(Raonwiz.Dext5.Process.Entity.UploadEventEntity parameterEntity) {
			...
			/**************** 다운로드 시 클라이언트에서 추가한 formdatavalue 값을 받는 부분 입니다. *****************/
			try {
				String[] FormDataValue = parameterEntity.getDownloadCustomValue(); //DownloadCustomValue
		...
		public void OpenDownloadCompleteEventEx(Raonwiz.Dext5.Process.Entity.UploadEventEntity parameterEntity) {
			...
			try {
				...
				if (FormDataValue != null && FormDataValue.length > 0) {
					...
					// downloadCustomValue.length는 zip 다운로드 하려는 파일 개수만큼 length가 정의 됩니다. (2개의 파일을 zip 다운로드 했다면 length = 2)
					for(int i=0; i<FormDataValue.length; i++){
						FormDataValue[i] = java.net.URLDecoder.decode(FormDataValue[i], "UTF-8");
	...
	*/
    ...
    String result = upload.Process(request, response, application, event); // 서버 이벤트 사용시 반드시 3번째 파라미터의 event 객체를 Setting 해줘야 합니다.
	...
	String result = "";
    try {
		result = upload.Process(request, response, application, event); // 서버 이벤트 사용시 반드시 3번째 파라미터의 event 객체를 Setting 해줘야 합니다.
	} catch (Exception e) {
		e.printStackTrace();
	}
