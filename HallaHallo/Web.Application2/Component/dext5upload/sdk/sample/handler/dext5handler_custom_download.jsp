		...
		public void OpenDownloadPathChangeEventEx(Raonwiz.Dext5.Process.Entity.UploadEventEntity parameterEntity) {
			...
			try {
				...
				for (int i = 0; i < downloadCustomValue.length; i++) {
					downloadCustomValue[i] = java.net.URLDecoder.decode(downloadCustomValue[i], "UTF-8");
					// Custom Value(AddUploadedFile 5번째 파라미터)를 이용하여 다운로드 받을 파일의 경로 설정
					if(downloadCustomValue[i].equals("CustomValue1")) {
	        			downloadFilePath[i] = TempPath + sPathChar + "images" + sPathChar + "Panorama" + sPathChar + "ViewPhotos.jpg";
					} else if(downloadCustomValue[i].equals("CustomValue2")) {
	        			downloadFilePath[i] = TempPath + sPathChar + "images" + sPathChar + "Scenery" + sPathChar + "image" + sPathChar + "CreativeImages.bmp";
					}
				}
