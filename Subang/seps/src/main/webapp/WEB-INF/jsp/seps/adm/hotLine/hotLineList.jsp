...
<script type="text/javascript">
	...
	// 조회
	function fn_select(hotLineId){
		$("#hotLineId").val(hotLineId);
		$("#frm").attr("method", "post").attr("action", "<c:out value='${pageContext.request.contextPath}/adm/hotLine/hotLineView.do'/>").submit();
	}
	
</script>
<body>
<form:form commandName="searchVO" id="frm" name="frm">
<input type="hidden" id="hotLineId" name="hotLineId">
	<!-- top menu - start -->
									...
									<c:forEach items="${resultList }" var="result" varStatus="status">
										<tr>
											...
											<td class="tal_l tint_10">
												<a href="#" onclick="fn_select(<c:out value='${result.hotLineId }'/>); return false;">
													<c:out value="${result.hotLineName }"/>
												</a>
											</td>
											<td><c:out value="${result.hotLineTel }"/></td>
											<td><c:out value="${result.hotLineEmail }"/></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
